...
class MPASDataCache:
    ...
    def __init__(
        self: "MPASDataCache", max_variables: int = 10, max_coordinates: int = 10
    ) -> None:
        """
    ...
    def __setstate__(self: "MPASDataCache", state: Dict[str, Any]) -> None:
        """
    ...
    def load_coordinates_from_dataset(
        self: "MPASDataCache", dataset: xr.Dataset, var_name: Optional[str] = None
    ) -> None:
        """
        ...
        with self._get_lock():
            ...
            if var_name and var_name in dataset.data_vars:
                ...
                if "nCells" in var_dims:
                    lon_coord_name, lat_coord_name = "lonCell", "latCell"
                elif "nVertices" in var_dims:
                    lon_coord_name, lat_coord_name = "lonVertex", "latVertex"
                elif "nEdges" in var_dims:
                    lon_coord_name, lat_coord_name = "lonEdge", "latEdge"
                else:
                    lon_coord_name, lat_coord_name = "lonCell", "latCell"
            else:
                # Default coordinates
            ...
            if lon_coord_name not in dataset or lat_coord_name not in dataset:
                raise ValueError(
    ...
    def get_coordinates(
        self: "MPASDataCache", var_name: Optional[str] = None
        ...
        with self._get_lock():
            ...
            if cache_key not in self._coordinates:
                raise KeyError(
                    f"Coordinates for '{cache_key}' not loaded. Call load_coordinates_from_dataset first."
                )
    ...
    def get_variable_data(
        ...
        with self._get_lock():
            ...
            if cache_key not in self._variables:
                raise KeyError(
                    f"Variable data '{cache_key}' not loaded. Call load_variable_data first."
                )
    ...
    def get_cache_info(self: "MPASDataCache") -> Dict[str, Any]:
        ...
        """
        with self._get_lock():
            total_memory = sum(var.data.nbytes for var in self._variables.values())
    ...
    @staticmethod
    def _make_cache_key(
        var_name: str, time_index: Optional[int], level_index: Optional[int]
    ) -> str:
        """
