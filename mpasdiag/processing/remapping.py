...
def _convert_coordinates_to_degrees(
    lon: Union[np.ndarray, xr.DataArray], lat: Union[np.ndarray, xr.DataArray]
...
class MPASRemapper:
    ...
    def prepare_source_grid(
        ...
        source_grid = xr.Dataset(
            {"lon": xr.DataArray(lon, dims=["x"]), "lat": xr.DataArray(lat, dims=["x"])}
        )

        if lon_bounds is not None and lat_bounds is not None:
            lon_bounds, lat_bounds = _convert_coordinates_to_degrees(
    ...
    def _resolve_grids(
        ...
        target_grid: Optional[xr.Dataset],
    ...
    def _try_load_cached_weights(
        self: "MPASRemapper", weights_path: Optional[Path]
    ) -> bool:
        """
    ...
    def _prepare_source_esmpy(
        self: "MPASRemapper",
        source_grid: xr.Dataset,
        source_lon_deg: np.ndarray,
        source_lat_deg: np.ndarray,
    ) -> Tuple[Any, int, bool, bool, Any, Optional[np.ndarray]]:
        """
        ...
        if is_conservative:
            ...
            lon_b = np.where(flat_lon_b < 0, flat_lon_b + 360, flat_lon_b).reshape(
                raw_lon_b.shape
            )
        ...
        if getattr(esmpy.RegridMethod, self._METHOD_MAP[self.method]) in {
            esmpy.RegridMethod.BILINEAR,
            esmpy.RegridMethod.PATCH,
        }:
            logger.warning(
    ...
    def _build_weights_on_rank0(
        ...
        source_lon_deg = np.where(
            source_lon_deg < 0, source_lon_deg + 360, source_lon_deg
        )
    ...
    def _sync_weights_across_ranks(
        self: "MPASRemapper",
        comm: Any,
        mpi_rank: int,
        weights_path: Optional[Path],
    ...
    def _broadcast_weights_in_memory(
        self: "MPASRemapper",
        comm: Any,
        mpi_rank: int,
    ) -> None:
        ...
        if mpi_rank == 0:
            ...
            bcast_coe: Optional[np.ndarray] = (
                self._cell_of_element.astype(np.int64)
                if self._cell_of_element is not None
        ...
        if mpi_rank != 0:
            ...
            bcast_coe = (
                np.empty(metadata["coe_len"], dtype=np.int64)
                if metadata["has_coe"]
                else np.empty(0, np.int64)
            )
        ...
        if mpi_rank != 0:
            ...
            self._weights = coo_matrix(
                (
                    weight_values,
                    (row_indices.astype(np.intp), col_indices.astype(np.intp)),
                ),
    ...
    def unstructured_to_structured_grid(
        ...
        intermediate_lons = np.arange(
            lon_min, lon_max + intermediate_resolution / 2, intermediate_resolution
        )

        intermediate_lats = np.arange(
            lat_min, lat_max + intermediate_resolution / 2, intermediate_resolution
        )

        lon_2d, lat_2d = np.meshgrid(
            intermediate_lons, intermediate_lats
        ...
        structured_data = xr.DataArray(
            data_2d,
            dims=["lat", "lon"],
            coords={"lon": intermediate_lons, "lat": intermediate_lats},
            attrs=data_attrs,
        )
        ...
        logger.info(
            "Converted to 2D structured grid with bounds (ready for conservative remapping)"
        )
    ...
    @staticmethod
    def _build_esmpy_locstream(lon_deg: np.ndarray, lat_deg: np.ndarray) -> Any:
        """
    ...
    def _build_esmpy_mesh(
        ...
        if nv in (3, 4):
            element_type = (
                esmpy.MeshElemType.TRI if nv == 3 else esmpy.MeshElemType.QUAD
            )
    ...
    @staticmethod
    def _build_esmpy_grid(
        lon_1d: np.ndarray, lat_1d: np.ndarray, add_corners: bool = False
    ) -> Tuple[Any, int, int]:
        """
        ...
        lon_2d, lat_2d = np.meshgrid(
            lon_1d, lat_1d
        )  # type: np.ndarray, np.ndarray  # each (n_lat, n_lon)
        grid.coords[esmpy.StaggerLoc.CENTER][0][
            :
        ] = lon_2d.T  # stored as (n_lon, n_lat)
        ...
        if add_corners:
            ...
            lon_b_2d, lat_b_2d = np.meshgrid(
                lon_b, lat_b
            )  # type: np.ndarray, np.ndarray  # (n_lat+1, n_lon+1)
    ...
    def _build_weights_from_esmpy(
        ...
        if extrap_method is not None:
            dist_to_src_method = getattr(
                esmpy.ExtrapMethod, "NEAREST_D2SRATIO", esmpy.ExtrapMethod.NEAREST_STOD
            )
            extrap_method_map = {
            ...
            extrapolation_method = extrap_method_map.get(
                extrap_method, esmpy.ExtrapMethod.NONE
            )
        ...
        weight_matrix = coo_matrix(
            (weight_values, (row, col)), shape=(n_tgt, n_src)
        ).tocsr()
    ...
    @staticmethod
    def _save_weights_netcdf(
        path: Path,
        weight_matrix: Any,
        n_src: int,
        tgt_shape: Tuple[int, int],
        method: str,
        cell_of_element: Optional[np.ndarray] = None,
    ) -> None:
        """
    ...
    @staticmethod
    def _load_weights_netcdf(
        path: Path,
    ) -> Tuple[Any, Tuple[int, int], Optional[np.ndarray]]:
        """
...
def _add_wrapped_boundary_points(
...
def remap_mpas_to_latlon(
    ...
    with warnings.catch_warnings():
        ...
        logger.debug(
            "  Mean: %.4f, Median: %.4f",
            float(np.nanmean(data_values)),
            float(np.nanmedian(data_values)),
        )
        logger.debug(
            "  Std: %.4f, Sum: %.4f",
            float(np.nanstd(data_values)),
            float(np.nansum(data_values)),
        )
    ...
    if np.abs(data_sum) < 1e-10:
        ...
        with warnings.catch_warnings():
            ...
            logger.error(
                "Data min: %s, max: %s",
                np.nanmin(data_values),
                np.nanmax(data_values),
            )
    ...
    logger.debug(
        "  Min: %.4f, Max: %.4f",
        float(result.min(skipna=True)),
        float(result.max(skipna=True)),
    )
    logger.debug(
        "  Mean: %.4f, Median: %.4f",
        float(result.mean(skipna=True)),
        float(np.nanmedian(result.values)),
    )
    logger.debug(
        "  Std: %.4f, Sum: %.4f",
        float(result.std(skipna=True)),
        result_sum,
    )
...
def _extract_cell_coordinates(dataset: xr.Dataset) -> Tuple[xr.DataArray, xr.DataArray]:
    ...
    raise ValueError(
        "Could not find cell coordinates (lonCell/latCell or lon/lat) in dataset"
    )


def _resolve_grid_bounds(
    dataset: xr.Dataset,
    ...
    lon_np, lat_np = MPASGeographicUtils.extract_spatial_coordinates(
...
def remap_mpas_to_latlon_with_masking(
    ...
    lon_coords = _apply_lon_convention(
        lon_coords, lon_data_range, lon_min, lon_max, lon_convention
    )
...
def dispatch_remap(
    ...
    raise ValueError(f"Unknown remap_engine '{engine}'. Must be 'kdtree' or 'esmf'.")
...
def create_target_grid(
    ...
    return xr.Dataset(
        {"lon": xr.DataArray(lon, dims=["lon"]), "lat": xr.DataArray(lat, dims=["lat"])}
    )
