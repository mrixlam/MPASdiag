...
class FileManager:
    ...
    @staticmethod
    def find_files(
        directory: str, pattern: str = "*.nc", recursive: bool = False
    ) -> List[str]:
        """
    ...
    @staticmethod
    def cleanup_files(
        directory: str, pattern: str = "*.tmp", older_than_days: int = 7
    ) -> int:
        """
    ...
    def _find_diag_files(data_dir: str) -> List[str]:
        ...
        if not files:
            ...
            if not files:
                files = FileManager.find_files(
                    mpasout_subdir, DIAG_GLOB, recursive=False
                )
