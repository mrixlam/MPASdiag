...
_WORKER_THREADS_ENV = "MPASDIAG_WORKER_THREADS"
_THREAD_ENV_VARS = (
    "OMP_NUM_THREADS",
...
def _configure_worker_threads(n_threads: int = 1) -> None:
    """
    This function caps the intra-process thread parallelism of a worker so that co-scheduled workers (MPI ranks or multiprocessing pool processes) do not oversubscribe the node. Without it, every worker's dask '.compute()' spins up a thread pool sized to ALL cores on the node and the BLAS inside NumPy/SciPy does the same, so N workers fight over N*cores threads and per-task time inflates as workers are added -- the anti-scaling signature. It pins dask to the synchronous scheduler (intra-rank dask threads only add overhead because netCDF4/HDF5 reads serialize behind xarray's global lock anyway), limits the already-loaded BLAS pools via threadpoolctl when available, and sets the standard thread-count environment variables for libraries that have not been imported yet. The process-wide MPASDIAG_WORKER_THREADS environment variable overrides the computed thread count, and the function is idempotent so the first caller wins.

    Parameters:
        n_threads (int): Number of threads this worker may use, typically cores_per_node // workers_per_node. Defaults to 1.

    Returns:
        None
    """
    global _worker_threads_configured, _threadpool_limiter

    if _worker_threads_configured:
        return

    _worker_threads_configured = True
    override = os.environ.get(_WORKER_THREADS_ENV)
...
def _multiprocessing_task_wrapper(
    args: Tuple[int, Any, Callable, str, Tuple, Dict],
) -> "TaskResult":
    """
    ...
    return _execute_task_basic(
        task_id, task, func, error_policy_value, func_args, func_kwargs
    )


def _execute_task_basic(
    task_id: int,
    task: Any,
    func: Callable,
    error_policy_value: str,
    func_args: Tuple,
    func_kwargs: Dict,
) -> "TaskResult":
    """
    This function executes a single task and packages the outcome into a TaskResult, forming the shared execution core for both the legacy per-task-argument multiprocessing wrapper and the initializer-based pool context wrapper. It runs the provided function on the task with the supplied positional and keyword arguments, records wall-clock execution time, and captures any exception (with traceback) on the result. Under the 'abort' error policy the original exception is re-raised so the pool surfaces the failure immediately; otherwise the error is recorded and execution continues, matching the collect/continue policies of the parallel manager.

    Parameters:
        task_id (int): Unique identifier of the task within the batch.
    ...
    Returns:
        TaskResult: Outcome of the task with success flag, result or error, and execution time.
    """
...
_POOL_TASK_CONTEXT: Optional[Tuple[Callable, str, Tuple, Dict]] = None
...
def _pool_worker_initializer(
    ...
    """
    This function initializes a multiprocessing pool worker process exactly once, installing the shared task context and containing the worker's thread usage. The task context (function, error policy, and the extra positional/keyword arguments shared by every task) is transferred a single time per worker instead of being pickled into every task tuple, which removes the per-task serialization of payloads such as the batch worker kwargs. It also calls _configure_worker_threads with the per-worker thread budget so pool workers do not collectively oversubscribe the node with dask and BLAS thread pools.

    Parameters:
        func (Callable): The function every task in this pool executes.
        error_policy_value (str): The error handling policy as a string ('abort', 'continue', 'collect').
        func_args (Tuple): Additional positional arguments shared by all tasks.
        func_kwargs (Dict): Additional keyword arguments shared by all tasks.
        n_pool_workers (int): Total pool size, used to derive the per-worker thread budget.

    Returns:
        None
    """
    global _POOL_TASK_CONTEXT
    _POOL_TASK_CONTEXT = (func, error_policy_value, func_args, func_kwargs)
    _configure_worker_threads(max(1, cpu_count() // max(1, n_pool_workers)))


def _pool_task_wrapper(indexed_task: Tuple[int, Any]) -> "TaskResult":
    """
    This function executes one task inside a pool worker using the context installed by _pool_worker_initializer, so each submitted task only carries its index and payload rather than a full copy of the function, policy, and shared arguments. It delegates to the shared single-task execution core so success/error semantics and timing match the legacy wrapper and the MPI execution path.

    Parameters:
        indexed_task (Tuple[int, Any]): Two-element tuple of (task_id, task payload).

    Returns:
        TaskResult: Outcome of the task with success flag, result or error, and execution time.
    """
    task_id, task = indexed_task
    assert _POOL_TASK_CONTEXT is not None, "Pool worker context not initialized"
    func, error_policy_value, func_args, func_kwargs = _POOL_TASK_CONTEXT
    return _execute_task_basic(
        task_id, task, func, error_policy_value, func_args, func_kwargs
    )
...
class MPASTaskDistributor:
    ...
    def __init__(
        self: "MPASTaskDistributor",
        comm: Any,
        strategy: LoadBalanceStrategy = LoadBalanceStrategy.DYNAMIC,
    ) -> None:
        """
    ...
    def distribute_tasks(
        self: "MPASTaskDistributor", tasks: List[Any]
    ) -> List[Tuple[int, Any]]:
        """
...
class MPASResultCollector:
    ...
    def __init__(self: "MPASResultCollector", comm: Any) -> None:
        """
    ...
    def gather_results(
        self: "MPASResultCollector", local_results: List[TaskResult]
    ...
    def compute_statistics(
        self: "MPASResultCollector", results: List[TaskResult]
    ) -> ParallelStats:
        """
        ...
        stats.failed_tasks = sum(
            1 for task_result in results if not task_result.success
        )
        stats.total_time = sum(task_result.execution_time for task_result in results)

        for result in results:
            worker_rank = result.worker_rank
            stats.worker_times[worker_rank] = (
                stats.worker_times.get(worker_rank, 0.0) + result.execution_time
            )
        ...
        if stats.worker_times:
            ...
            stats.load_imbalance = (
                (max_worker_time - avg_worker_time) / avg_worker_time
...
class MPASParallelManager:
    ...
    _TAG_REQUEST = (
        11  # worker -> master: ready for work (carries previous result or None)
    )
    ...
    def _setup_mpi_backend(
        self: "MPASParallelManager",
        load_balance_strategy: Union[str, "LoadBalanceStrategy"],
    ) -> None:
        """
        ...
        try:
            ...
            if self.size > 1:
                ...
                self._contain_mpi_rank_threads()
            else:
    ...
    def _contain_mpi_rank_threads(self: "MPASParallelManager") -> None:
        """
        This method derives this rank's thread budget from the node layout and applies it via _configure_worker_threads, so co-located MPI ranks share the node's cores instead of each spawning a full-node dask/BLAS thread pool. It discovers how many ranks share this rank's physical node by exchanging processor names across the communicator, divides the node's cores evenly among them, and pins dask to the synchronous scheduler plus the BLAS pools to the computed budget. Any failure (e.g. an MPI build without Get_processor_name) falls back to a conservative single-thread budget, which is always safe.
        ...
        try:
            ...
            _configure_worker_threads(max(1, cpu_count() // max(1, ranks_on_node)))
        except Exception:
            _configure_worker_threads(1)
    ...
    def set_error_policy(
        self: "MPASParallelManager", policy: Union[str, ErrorPolicy]
    ) -> None:
        """
    ...
    def parallel_map(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            and self.is_master
            and self.size > 1
            and 0 < len(tasks) < 2 * self.size
        ):
            logger.info(
                "Task granularity: %d tasks across %d workers -- additional workers "
                "beyond %d cannot reduce the runtime of this batch",
                len(tasks),
                self.size,
                len(tasks),
            )
    ...
    def _mpi_map(
        ...
        **kwargs: Any,
        ...
        if use_work_stealing:
            ...
            local_results = self._execute_local_tasks(
                func, local_tasks, *args, **kwargs
            )
    ...
    def _mpi_dispatch_master(
        self: "MPASParallelManager", n_tasks: int
    ) -> List[TaskResult]:
        """
    ...
    def _mpi_dispatch_worker(
        ...
        **kwargs: Any,
        ...
        while True:
            ...
            result = self._execute_one_task(
                func, task_id, tasks[task_id], *args, **kwargs
            )
    ...
    def _run_pool_with_fallback(
        ...
        """
        This method attempts to execute the indexed tasks in parallel using a multiprocessing pool, transferring the shared task context (function, error policy, and shared arguments) once per worker through the pool initializer instead of pickling it into every task. It tries the platform-appropriate multiprocessing start methods in order (fork, forkserver, spawn) and, within a pool, uses imap_unordered with a small chunksize so that free workers immediately pick up remaining tasks -- dynamic load balancing that pool.map's large static chunks cannot provide when per-task durations vary. Results are re-sorted by task id so callers keep the original ordering contract. If every multiprocessing method fails (e.g., due to platform limitations or unpicklable functions), it falls back to executing the tasks serially in-process, ensuring that all tasks are executed regardless.

        Parameters:
            func (Callable): The function to execute on each task payload.
            indexed_tasks (List[Tuple[int, Any]]): List of (task_id, task payload) tuples to execute.
            func_args (Tuple): Additional positional arguments shared by all tasks.
            func_kwargs (Dict): Additional keyword arguments shared by all tasks.

        Returns:
            List[TaskResult]: A list of TaskResult objects ordered by task id, each including success status, result data, error messages, and execution time. The results cover every input task regardless of whether multiprocessing was successful or the method had to fall back to serial execution.
        """
        context_methods = self._get_mp_context_methods()
        error_policy_value = self.error_policy.value
        initargs = (func, error_policy_value, func_args, func_kwargs, self.size)
        chunksize = max(1, len(indexed_tasks) // (max(1, self.size) * 8))
        ...
        for context_method in context_methods:
            try:
                ...
                results.sort(key=lambda task_result: task_result.task_id)
                return results
            except Exception as e:
                if self.verbose:
                    logger.warning(
                        "Multiprocessing with '%s' failed: %s",
                        context_method,
                        e,
                    )
    ...
    def _multiprocessing_map(
        ...
        **kwargs: Any,
        ...
        results = self._run_pool_with_fallback(func, indexed_tasks, args, kwargs)
    ...
    def _serial_map(
        ...
        **kwargs: Any,
        ...
        self.stats = (
            self.collector.compute_statistics(results) if self.collector else None
        )
...
def parallel_plot(
    plot_function: Callable, files: List[str], **kwargs: Any
) -> Optional[List[TaskResult]]:
    """
...
if __name__ == "__main__":
    ...
    def test_function(task_id: int, delay: float = 0.1) -> str:
