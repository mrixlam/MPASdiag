...
class MPASBaseProcessor:
    ...
    def _find_files_by_pattern(
        self: "MPASBaseProcessor", data_dir: str, pattern: str, file_type: str
    ) -> List[str]:
        """
        ...
        if not files:
            raise FileNotFoundError(
                f"No {file_type} found matching pattern: {file_pattern}"
            )
    ...
    def validate_files(self: "MPASBaseProcessor", files: List[str]) -> List[str]:
        """
    ...
    def _prepare_chunking_config(
        self: "MPASBaseProcessor", chunks: Optional[dict]
    ) -> tuple[dict, dict]:
        """
        ...
        """
        # Specify default chunking strategy when no custom chunks are provided.
        if chunks is None:
    ...
    def _apply_chunking(
        self: "MPASBaseProcessor", dataset: xr.Dataset, chunks: Optional[dict]
    ) -> xr.Dataset:
        """
    ...
    def _create_uxarray_dataset(
        self: "MPASBaseProcessor", combined_ds: xr.Dataset, data_files: List[str]
    ) -> ux.UxDataset:
        """
    ...
    def _attempt_primary_load(
        ...
        combined_ds = self._load_multifile_dataset(
            data_files, file_datetimes, open_chunks, drop_variables=drop_variables
        )
        ...
        if use_pure_xarray:
            ...
            if self.verbose:
                self._print_loading_success(
                    len(data_files), combined_ds, "pure xarray", data_type_label
                )
            ...
            if self.verbose:
                self._print_loading_success(
                    len(data_files), final_ds, "UXarray", data_type_label
                )
    ...
    def _attempt_fallback_load(
        ...
        combined_ds = self._load_multifile_dataset(
            data_files, file_datetimes, fallback_chunks, drop_variables=drop_variables
        )
        ...
        if self.verbose:
            self._print_loading_success(
                len(data_files), combined_ds, "xarray (fallback)", data_type_label
            )
    ...
    def _load_data(
        ...
        file_datetimes = MPASDateTimeUtils.parse_file_datetimes(
            data_files, self.verbose
        )
    ...
    def _compute_drop_variables(
        self: "MPASBaseProcessor", data_files: List[str], variables: Optional[List[str]]
    ) -> Optional[List[str]]:
        """
    ...
    def _attempt_load_with_fallbacks(
        self: "MPASBaseProcessor",
        data_files: List[str],
        file_datetimes: Any,
        open_chunks: Optional[dict],
        ...
        reference_file: str,
        data_type_label: str,
        drop_variables: Optional[List[str]],
    ) -> Tuple[Any, str]:
        """
        ...
        try:
            ...
            if self.verbose:
                logger.warning(
                    "Primary %s loading failed: %s", data_type_label.lower(), e
                )
                logger.warning(
                    "Trying xarray fallback for %s data", data_type_label.lower()
                )
    ...
    def _select_fallback_file(
        self: "MPASBaseProcessor", reference_file: str, data_files: List[str]
    ) -> str:
        """
    ...
    def _load_single_file_uxarray(
        self: "MPASBaseProcessor", file_path: str
    ) -> ux.UxDataset:
        """
        ...
        return ux.open_dataset(
            self.grid_file, file_path, grid_kwargs={"decode_times": False}
        )

    def _load_single_file_xarray(
        self: "MPASBaseProcessor", file_path: str
    ) -> xr.Dataset:
        """
    ...
    def _load_single_file_fallback(
        self: "MPASBaseProcessor", reference_file: str, data_files: List[str]
    ) -> Tuple[Any, str]:
        """
        ...
        if self.verbose:
            logger.warning(
                "Falling back to single-file loading (limited functionality)"
            )
    ...
    def normalize_longitude(
        self: "MPASBaseProcessor", lon: Union[float, np.ndarray]
    ...
    def validate_geographic_extent(
        self: "MPASBaseProcessor", extent: Tuple[float, float, float, float]
    ) -> bool:
        """
    ...
    def extract_spatial_coordinates(
        ...
        if lon_coords is None or lat_coords is None:
            available_vars = list(self.dataset.coords.keys()) + list(
                self.dataset.data_vars.keys()
            )
            raise ValueError(
                f"Could not find spatial coordinates. Available variables: {available_vars}"
            )
        ...
        assert isinstance(
            lon_coords_normalized, np.ndarray
        ), "Expected array from normalize_longitude"

        return lon_coords_normalized, lat_coords_flat

    def _load_grid_file(
        self: "MPASBaseProcessor", needed_vars: Optional[List[str]] = None
    ) -> xr.Dataset:
        """
    ...
    def _prepare_dimension_coordinates(
        self: "MPASBaseProcessor", combined_ds: xr.Dataset, dimensions_to_add: List[str]
    ) -> dict:
        """
        ...
        for dim_name in dimensions_to_add:
            if dim_name in combined_ds.sizes:
                coords_to_add[dim_name] = (
                    dim_name,
                    np.arange(combined_ds.sizes[dim_name]),
                )
                if self.verbose:
                    logger.debug(
                        ...
                        dim_name,
                        dim_name,
                        combined_ds.sizes[dim_name],
                    )

        return coords_to_add

    def _prepare_spatial_variables(
        self: "MPASBaseProcessor",
        ...
        for var_name in spatial_vars:
            if (
                var_name in grid_file_ds.variables
                and var_name not in combined_ds.data_vars
            ):
    ...
    def _apply_coordinate_updates(
        self: "MPASBaseProcessor",
        combined_ds: xr.Dataset,
        coords_to_add: dict,
        data_vars_to_add: dict,
    ) -> xr.Dataset:
        """
        ...
        if coords_to_add:
            ...
            if self.verbose:
                logger.debug(
                    "Successfully added %d coordinate variables", len(coords_to_add)
                )
        ...
        if data_vars_to_add:
            ...
            if self.verbose:
                logger.debug(
                    "Successfully added %d spatial variables", len(data_vars_to_add)
                )
                logger.debug(
                    "Updated dataset coordinates: %s", list(combined_ds.coords.keys())
                )
        else:
            if self.verbose:
    ...
    def _add_spatial_coords_helper(
        ...
        try:
            ...
            combined_ds = self._apply_coordinate_updates(
    ...
    def get_time_info(
        self: "MPASBaseProcessor", time_index: int, var_context: str = ""
    ) -> str:
        """
        ...
        return MPASDateTimeUtils.get_time_info(
            self.dataset, time_index, var_context, self.verbose
        )

    def parse_file_datetimes(self: "MPASBaseProcessor", diag_files: List[str]) -> List:
        """
    ...
    def validate_time_parameters(
        self: "MPASBaseProcessor", time_index: int
    ) -> Tuple[str, int, int]:
        """
        ...
        return MPASDateTimeUtils.validate_time_parameters(
            self.dataset, time_index, self.verbose
        )

    def filter_by_spatial_extent(
        self: "MPASBaseProcessor",
        data: Any,
        lon_min: float,
        lon_max: float,
