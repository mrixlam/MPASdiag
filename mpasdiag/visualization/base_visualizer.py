...
class MPASVisualizer:
    ...
    def format_latitude(self: "MPASVisualizer", value: float, _: Any) -> str:
        """
    ...
    def format_longitude(self: "MPASVisualizer", value: float, _: Any) -> str:
        """
    ...
    def calculate_adaptive_marker_size(
        ...
        """
        return MPASVisualizationStyle.calculate_adaptive_marker_size(
            map_extent, num_points, fig_size
        )

    def _format_ticks_dynamic(self: "MPASVisualizer", ticks: List[float]) -> List[str]:
        """
    ...
    def get_variable_specific_settings(
        self: "MPASVisualizer", var_name: str, data: np.ndarray
    ...
    def add_regional_features(
        ...
        if lat_min >= 15 and lat_max <= 60 and lon_min >= -135 and lon_max <= -55:
            show_states = True

        if show_states and isinstance(self.ax, GeoAxes):
            self.ax.add_feature(
                cfeature.STATES, linewidth=0.5, edgecolor="red", facecolor="none"
            )
    ...
    def save_plot(
        ...
        MPASVisualizationStyle.save_plot(
            self.fig, output_path, formats, bbox_inches, pad_inches, self.dpi
        )
    ...
    def create_histogram(
        ...
        if len(valid_data) > 0:
            ...
            _, _hist_bin_edges, _ = self.ax.hist(
                valid_data, bins=bins_arg, alpha=0.7, edgecolor="black", linewidth=0.5
            )
            ...
            self.ax.axvline(
                mean_val + std_val, color="orange", linestyle=":", linewidth=1
            )
    ...
    def extract_2d_from_3d(
        ...
        """
        return MPAS3DProcessor.extract_2d_from_3d(
            data_3d, level_index, level_value, level_dim, method
        )

    @staticmethod
    def _subsample_wind_vectors(
        lon_filtered: np.ndarray,
        ...
        if subsample > 1:
            ...
            return (
                lon_filtered[subsample_indices],
                lat_filtered[subsample_indices],
                u_filtered[subsample_indices],
                v_filtered[subsample_indices],
            )
    ...
    def create_wind_plot(
        ...
        self.ax.add_feature(
            cfeature.BORDERS, linewidth=0.5, edgecolor="gray", alpha=0.7
        )
        ...
        if not np.any(valid_wind_mask):
            raise ValueError(
                "No valid wind data points found within the specified map extent."
            )
        ...
        logger.debug(
            ...
            float(np.min(wind_speed_filtered)),
            float(np.max(wind_speed_filtered)),
        )

        if show_background:
            self._create_wind_background(
                lon_filtered, lat_filtered, wind_speed_filtered, bg_colormap, data_crs
            )
        ...
        if plot_type.lower() == "barbs":
            ...
            raise ValueError(
                f"plot_type must be 'barbs' or 'arrows', got '{plot_type}'"
            )
    ...
    def draw_transect_line(
        ...
        style: Optional[TransectLineStyle] = None,
    ) -> Dict[str, Any]:
        """
        ...
        if isinstance(ax, GeoAxes):
            plot_kwargs["transform"] = (
                data_crs if data_crs is not None else ccrs.PlateCarree()
            )
        ...
        if show_labels and start_label is not None:
            start_text = ax.text(
                slon + xoffset, slat + yoffset, start_label, **text_kwargs
            )
    ...
    def _create_wind_background(
        ...
        marker_size = (
            self.calculate_adaptive_marker_size(
        ...
        scatter_collection = self.ax.scatter(
            lon,
            lat,
            c=wind_speed,
            cmap=colormap,
        ...
        colorbar = MPASVisualizationStyle.add_colorbar(
            ...
            self.ax,
            scatter_collection,
            label=MPASVisualizationStyle.build_colorbar_label(
    ...
    def _add_gridlines(self: "MPASVisualizer", data_crs: ccrs.CRS) -> None:
        """
    ...
    def _has_boundary_data(dataset: Optional[xr.Dataset]) -> bool:
        ...
        """
        return dataset is not None and "lon_b" in dataset and "lat_b" in dataset

    def _ensure_boundary_data(
        self: "MPASVisualizer", dataset: Optional[xr.Dataset]
    ) -> Optional[xr.Dataset]:
        """
        ...
        vertices_on_cell = dataset[
            "verticesOnCell"
        ].values  # (nCells, maxEdges), 1-indexed
        ...
        if self._bounds_cache_key != cache_key:
            ...
            for i in range(n_cells):
                ...
                padded_indices = np.minimum(
                    np.arange(most_common_vertex_count), num_vertices - 1
                )
    ...
    def _extract_full_grid(dataset: xr.Dataset) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for lon_key in ("lonCell", "lon", "longitude"):
            ...
            raise KeyError(
                "Cannot find longitude coordinate (lonCell/lon/longitude) in dataset"
            )
        for lat_key in ("latCell", "lat", "latitude"):
            ...
            raise KeyError(
                "Cannot find latitude coordinate (latCell/lat/latitude) in dataset"
            )
    ...
    def _backmap_to_full_grid(
        ...
        nearest_distances, nearest_indices = tree.query(
            np.column_stack([lon_full, lat_full])
        )
    ...
    def _get_or_build_remapper(
        ...
        if self._remapper is None or self._remapper_key != cache_key:
            ...
            remapper.prepare_source_grid(
                lon_full, lat_full, lon_bounds=lon_b, lat_bounds=lat_b
            )
    ...
    def _interpolate_to_grid(
        ...
        use_esmf = engine == "esmf" or (  # explicit engine request
            engine is None  # legacy auto-select
            and ESMPY_AVAILABLE
    ...
    @staticmethod
    def _determine_grid_resolution(
        grid_resolution: Optional[float],
        lon_min: float,
        lon_max: float,
        lat_min: float,
        lat_max: float,
        resolution_bounds: Optional[Tuple[float, float]],
    ) -> float:
        """
    ...
    def _attempt_esmf_remap(
        ...
        try:
            ...
            lon_mesh, lat_mesh = np.meshgrid(
                remap_result.lon.values, remap_result.lat.values
    ...
    def _kdtree_interpolate(
        ...
        """
        logger.info(
            "Interpolating %d points using KDTree (%s)", len(data), kdtree_method
        )
        ...
        logger.debug(
            "KDTree produced %dx%d grid", data_interp.shape[0], data_interp.shape[1]
        )
