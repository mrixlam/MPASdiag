...
class MPASSurfacePlotter(MPASVisualizer):
    ...
    @staticmethod
    def _apply_level_index_slice(
        data: Union[np.ndarray, xr.DataArray], level_index: int
    ...
    def _extract_2d_from_multidimensional(
        ...
        if data.ndim > 3:
            raise ValueError(
                f"only 1D, 2D and 3D data are supported, got {data.ndim}D data with shape {data.shape}"
            )
        ...
        if data.ndim > 1:
            data = data.flatten()
            logger.debug(
                "Flattened remaining dimensions to 1D, final shape: %s", data.shape
            )
    ...
    def _extract_and_convert_units(
        ...
        original_unit = (
            file_unit
            if file_unit
            else var_metadata.get("original_units") or var_metadata.get("units")
        )
        ...
        if original_unit != display_unit:
            try:
                ...
                logger.debug(
                    "Converted %s from %s to %s", var_name, original_unit, display_unit
                )
    ...
    def _prepare_colormap_and_levels(
        ...
        final_colormap: str = (
            ...
            else var_metadata.get("colormap", "viridis")
        )
        ...
        if clim_min is not None and clim_max is not None and levels is not None:
            levels = [
                ...
                if clim_min <= level_value <= clim_max
            ]
    ...
    def _build_boundary_norm(
        ...
        try:
            if levels is not None:
                color_levels_sorted = sorted(
                    {level_value for level_value in levels if np.isfinite(level_value)}
                )
                if color_levels_sorted:
                    last_bound = max(color_levels_sorted) + 1
                    bounds = (
                        [min(color_levels_sorted)] + color_levels_sorted + [last_bound]
                    )
    ...
    def _filter_valid_data(
        ...
        if plot_type == "scatter":
            valid_mask = (
                ...
                & np.isfinite(lon)
                & np.isfinite(lat)
                & (lon >= filter_lon_min_data)
                & (lon <= filter_lon_max_data)
                & (lat >= filter_lat_min_data)
                & (lat <= filter_lat_max_data)
            )
        else:
        ...
        if not np.any(valid_mask):
            raise ValueError(
                f"No valid data points found within the specified map extent for {var_name}"
            )
        ...
        logger.debug(
            ...
            float(data_valid.min()),
            float(data_valid.max()),
            var_metadata["units"],
        )
    ...
    def _render_plot(
        ...
        if plot_type == "scatter":
            ...
            self._create_scatter_plot(
                lon_valid, lat_valid, data_valid, cmap_obj, norm, data_crs
            )
    ...
    def _generate_title(
        ...
        if title is None:
            ...
            if time_stamp:
                ...
                time_str = time_stamp.strftime(self._TIME_FMT)
                time_in_title = (
                    time_str in title or "Valid Time:" in title or "Valid:" in title
                )
        ...
        if wind_overlay is not None:
            try:
                ...
                wind_plotter.add_wind_overlay(
                    self.ax, lon, lat, wind_overlay, config=config
                )
        ...
        if surface_overlay is not None:
            try:
                self._add_surface_overlay(
                    ...
                    lon_min=filter_lon_min,
                    lon_max=filter_lon_max,
                    lat_min=filter_lat_min,
                    lat_max=filter_lat_max,
                    dataset=dataset,
                    config=config,
                )
    ...
    def create_surface_map(
        ...
        if plot_type not in ["scatter", "contour", "both", "contourf"]:
            raise ValueError(
                f"plot_type must be 'scatter', 'contour', 'contourf', or 'both', got '{plot_type}'"
            )
        ...
        ), "Figure must be created by _setup_map_extent_and_features"
        assert (
            self.ax is not None
        ), "Axes must be created by _setup_map_extent_and_features"
    ...
    def _infer_overlay_units(
        self: "MPASSurfacePlotter", var_name: str, overlay_data: np.ndarray
    ) -> Optional[str]:
        """
    ...
    def _apply_overlay_unit_conversion(
        self: "MPASSurfacePlotter",
            ...
            try:
                overlay_data = self.convert_to_numpy(
                    UnitConverter.convert_units(
                        overlay_data, original_units, display_units
                    )
                )
                logger.debug(
    ...
    def _prepare_overlay_data(
        ...
        if original_units:
            overlay_data = self._apply_overlay_unit_conversion(
                overlay_data, var_name, original_units
            )
    ...
    def _interpolate_overlay(
        ...
        if config is not None:
            remapped_overlay = dispatch_remap(
                data=data_xr,
                dataset=dataset,
                config=config,
                lon_min=lon_min,
        ...
        lon_mesh, lat_mesh = np.meshgrid(
            remapped_overlay.lon.values, remapped_overlay.lat.values
    ...
    def _validate_contour_levels(
        ...
        data_min, data_max = float(np.nanmin(data_interp)), float(
            np.nanmax(data_interp)
        )
        logger.debug(
            "Overlay data range: %.2f to %.2f", float(data_min), float(data_max)
        )
        ...
        if levels is not None:
            levels_in_range = [
                level_value
                for level_value in levels
                if data_min <= level_value <= data_max
            ]
    ...
    def _render_overlay(
        ...
        common_kwargs = {"transform": ccrs.PlateCarree(), "alpha": alpha}
        ...
        if plot_type == "contour":
            ...
            contour_lines = ax.contour(
                lon_mesh, lat_mesh, data_interp, **contour_kwargs
            )
            ...
            overlay_cmap = plt.get_cmap(colormap) if colormap else None
            contourf_kwargs = {"cmap": overlay_cmap, **common_kwargs}
    ...
    def _add_surface_overlay(
        ...
        if plot_type not in ["contour", "contourf"]:
            raise ValueError(
                f"Unsupported surface overlay plot_type: {plot_type}. Must be 'contour' or 'contourf'."
            )
        ...
        self._validate_contour_levels(
            data_interp, surface_config.get("levels"), var_name
        )
        ...
        for time_idx in range(total_times):
            try:
                ...
                output_path = os.path.join(
                    output_dir, f"{file_prefix}_{var_name}_{plot_type}_valid_{time_str}"
                )
                ...
                logger.exception(
                    "Error creating surface map for time index %d: %s", time_idx, e
                )
                continue
    ...
    def create_simple_scatter_plot(
        ...
        if not np.any(valid_mask):
            raise ValueError(
                "No valid data points found after filtering for finite values"
            )
        ...
        scatter = self.ax.scatter(
            lon_valid, lat_valid, c=data_valid, cmap=colormap, s=point_size, alpha=0.8
        )
    ...
    def plot_3d_variable_slice(
        ...
        metadata = MPASFileMetadata.get_3d_variable_metadata(
            var_name, slice_2d, level=level
        )
        colormap, levels = MPASFileMetadata.get_3d_colormap_and_levels(
            var_name, data_array, level=level
        )
        default_title = (
            f"{metadata.get('long_name', var_name)} — Model Level {level_idx}"
        )
...
def create_surface_plot(
    ...
    **kwargs: Any,
    ...
    style = SurfaceMapStyle(
        title=title, plot_type=plot_type, colormap=colormap, **style_kwargs
    )
