...
class MPASVisualizationStyle:
    ...
    def _resolve_accum_period(var_lower: str) -> str:
        ...
        if accum_match:
            return MPASVisualizationStyle._hours_to_accum_period(
                int(accum_match.group(1))
            )
    ...
    @staticmethod
    def get_variable_style(
        var_name: str, data_array: Optional[xr.DataArray] = None
    ) -> Dict[str, Any]:
        """
        ...
        style: Dict[str, Any] = cast(
            Dict[str, Any],
            style_configs.get(
                var_name.lower(), {"colormap": "viridis", "extend": "both"}
            ),
        )

        var_lower = var_name.lower()
        is_precipitation = style.get("use_precip_colormap") or any(
            precip_key in var_lower for precip_key in ["precip", "rain", "snow", "qpf"]
        )
        ...
        if data_array is not None and "levels" not in style and not is_precipitation:
            try:
                data_levels = MPASVisualizationStyle._generate_levels_from_data(
                    data_array, var_name
                )
    ...
    @staticmethod
    def _generate_levels_from_data(
        data_array: xr.DataArray, var_name: str
    ) -> Optional[List[float]]:
        """
        ...
        try:
            ...
            if hasattr(data_values, "flatten"):
                ...
                if any(x in var_name.lower() for x in ["temp", "t2m"]):
                    ...
                    levels: List[float] = cast(
                        List[float], np.arange(start, end + step, step).tolist()
                    )
                else:
                    n_levels = 15
                    level_step = (data_max - data_min) / n_levels
    ...
    @staticmethod
    def _levels_for_temperature(
        data_min: float, data_max: float, data_range: float
        ...
        levels = [
            float(x)
            for x in range(
                int(data_min // step) * step,
                int(data_max // step) * step + step * 2,
                step,
            )
        ]
        levels = [
            level_value for level_value in levels if data_min <= level_value <= data_max
        ]
        return colormap, levels

    @staticmethod
    def _levels_for_precipitation(
        var_lower: str, data_max: float
    ) -> Tuple[mcolors.ListedColormap, List[float]]:
        """
        ...
        if data_max > 0:
            levels = [
                level_value for level_value in levels if level_value <= data_max * 1.2
            ]
        return cmap, levels

    @staticmethod
    def _levels_for_pressure(
        var_lower: str, data_min: float, data_max: float, data_range: float
    ) -> Tuple[str, List[float]]:
        """
        ...
        if "slp" in var_lower or "mslp" in var_lower:
            ...
            step = (
                max(100, int(data_range / 20))
                if data_range > 1000
                else max(10, int(data_range / 15))
            )
    ...
    def _levels_for_wind(data_max: float) -> Tuple[str, List[float]]:
        ...
        levels = [
            level_value
            for level_value in candidate_levels
            if level_value <= data_max * 1.1
        ]
        return colormap, levels

    @staticmethod
    def _levels_for_geopotential(
        data_min: float, data_max: float, data_range: float
    ) -> Tuple[str, List[float]]:
        """
    ...
    @staticmethod
    def _levels_for_humidity(
        var_lower: str, data_min: float, data_max: float
    ) -> Tuple[str, List[float]]:
        """
    ...
    @staticmethod
    def get_variable_specific_settings(
        var_name: str, data: np.ndarray
    ) -> Tuple[Union[str, mcolors.ListedColormap], Optional[List[float]]]:
        """
        ...
        if any(k in var_lower for k in ["temp", "t2m", "temperature", "sst"]):
            return MPASVisualizationStyle._levels_for_temperature(
        ...
        if any(k in var_lower for k in ["pressure", "slp", "mslp", "pres"]):
            return MPASVisualizationStyle._levels_for_pressure(
                var_lower, data_min, data_max, data_range
            )
        ...
        if any(k in var_lower for k in ["geopotential", "height", "hgt", "z"]):
            return MPASVisualizationStyle._levels_for_geopotential(
                data_min, data_max, data_range
            )

        if any(k in var_lower for k in ["humidity", "rh", "q", "mixing"]):
            return MPASVisualizationStyle._levels_for_humidity(
                var_lower, data_min, data_max
            )
    ...
    def add_timestamp_and_branding(fig: Optional[Figure]) -> None:
        ...
        try:
            ...
            footer_ax = MPASVisualizationStyle._create_footer_axes(
                fig, height=0.7, pad=0.02
            )
    ...
    @staticmethod
    def _create_footer_axes(
        fig: Figure, height: float = 0.07, pad: float = 0.02
    ) -> Axes:
        """
        ...
        try:
            for ax in fig.axes:
                ...
                if (
                    axes_position.width > 0.35
                    and axes_position.height < 0.12
                    and axes_position.y0 < 0.35
                ):
        ...
        if colorbar_tops:
            ...
            logger.debug(
                "Detected colorbar tops=%s, placing footer bottom=%s",
                colorbar_tops,
                bottom,
            )
        else:
        ...
        footer_ax = fig.add_axes(
            (left, bottom, width, height), frameon=False, zorder=105
        )
    ...
    @staticmethod
    def _configure_horizontal_colorbar(
        cbar: Colorbar,
        label: Optional[str],
        label_pos: str,
        tick_labelsize: int,
        ...
        try:
            ...
            cbar.ax.xaxis.set_ticks_position(
                cast(Literal["top", "bottom"], tick_position)
            )
        except Exception:
            pass
        if label is not None:
            try:
                cbar.set_label(
                    label, fontsize=max(10, tick_labelsize), labelpad=labelpad
                )
            except Exception:
                cbar.ax.set_xlabel(
                    label, fontsize=max(10, tick_labelsize), labelpad=labelpad
    ...
    @staticmethod
    def _configure_vertical_colorbar(
        cbar: Colorbar,
        label: Optional[str],
        label_pos: str,
        tick_labelsize: int,
        ...
        try:
            ...
            cbar.ax.yaxis.set_ticks_position(
                cast(Literal["left", "right"], tick_position)
            )
        except Exception:
            pass
        if label is not None:
            try:
                cbar.set_label(
                    label, fontsize=max(8, tick_labelsize), labelpad=labelpad
                )
            except Exception:
                cbar.ax.set_ylabel(
                    label, fontsize=max(8, tick_labelsize), labelpad=labelpad
    ...
    @staticmethod
    def _apply_colorbar_formatter(cbar: Colorbar, fmt: Any) -> None:
        """
        ...
        try:
            cbar.formatter = (
                mticker.FormatStrFormatter(fmt) if isinstance(fmt, str) else fmt
            )
            cbar.update_ticks()
        except Exception:
            pass
    ...
    @staticmethod
    def build_colorbar_label(
        var_metadata: Optional[Dict[str, Any]] = None,
    ...
    @staticmethod
    def calculate_adaptive_marker_size(
    ...
    @staticmethod
    def _choose_tick_fmt(tick_values: np.ndarray, nonzero_ticks: np.ndarray) -> str:
        """
    ...
    @staticmethod
    def _resolve_duplicate_labels(fmt: str, ticks: List[float]) -> List[str]:
        """
        ...
        """
        precision_upgrade_map: Dict[str, str] = {
            "{:.0f}": "{:.1f}",
            "{:.1f}": "{:.2f}",
            "{:.2f}": "{:.3f}",
        }
    ...
    def format_ticks_dynamic(ticks: List[float]) -> List[str]:
        ...
        if len(set(formatted_labels)) < len(formatted_labels):
            formatted_labels = MPASVisualizationStyle._resolve_duplicate_labels(
                tick_fmt, ticks
            )
