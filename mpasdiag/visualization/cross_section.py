...
class MPASVerticalCrossSectionPlotter(MPASVisualizer):
    ...
    def __init__(self, figsize: Tuple[float, float] = (10, 12), dpi: int = 100):
        """
        Initializes the vertical cross-section plotter with figure dimensions and resolution settings
        for creating 2D slices through 3D atmospheric data. This constructor inherits from MPASVisualizer
        to establish the base plotting framework and configures default figure size optimized for vertical
        cross-section aspect ratios (taller than wide) and high-resolution output suitable for publication
        and detailed analysis of atmospheric vertical structure.
    ...
    def create_vertical_cross_section(self, 
        ...
        """
        Creates a vertical cross-section plot showing the vertical structure of a 3D atmospheric variable
        along a great-circle transect between two geographic points. This method generates interpolated
        cross-section data along the specified path, converts vertical coordinates to height when possible,
        applies automatic unit conversion and variable-specific styling, and produces filled contour or
        contour plots with professional formatting including colorbar, gridlines, title with statistics,
        and optional path annotation. It returns a complete figure ready for display or saving.
    ...
    def _generate_cross_section_data(self, 
        ...
        """
        Generates cross-section data by interpolating 3D atmospheric data along a great-circle path
        between start and end points. This internal method creates a transect path, extracts vertical
        levels from the dataset (pressure, height, or model levels), iterates through each vertical
        level to extract and interpolate horizontal data onto the path using nearest-neighbor KDTree
        interpolation, and assembles a complete 2D cross-section array. It returns a dictionary containing
        distances, vertical coordinates, interpolated data values, and path coordinates for plotting.
    ...
    def _generate_great_circle_path(self, 
                                  start_point: Tuple[float, float],
                                  end_point: Tuple[float, float],
                                  num_points: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Generates a great-circle path between two geographic points using spherical geometry and
        haversine distance calculation. This internal method computes the shortest path on a sphere
        between start and end points, calculates total distance using the haversine formula with
        Earth radius of 6371 km, performs spherical linear interpolation (slerp) to generate evenly
        spaced points along the great circle, and returns arrays of longitudes, latitudes, and
        cumulative distances suitable for cross-section path definition.
    ...
    def _interpolate_along_path(self, 
        ...
        """
        Interpolates unstructured grid data along a specified path using nearest-neighbor spatial search
        with KDTree acceleration. This internal method converts geographic coordinates to 3D Cartesian
        coordinates for accurate spherical distance calculations, filters out NaN values from the source
        grid data, builds a KDTree spatial index for efficient nearest-neighbor queries, and interpolates
        values at each path point by finding the closest valid grid cell. It returns an array of interpolated
        values matching the path length.
    ...
    def _get_default_levels(self, data_values: Union[np.ndarray, xr.DataArray, Any], var_name: str) -> np.ndarray:
        """
        Generates default contour levels for a variable based on data range and variable type with
        context-aware spacing strategies. This internal method analyzes the data distribution excluding
        NaN values, applies variable-specific level generation rules (temperature uses fixed intervals,
        pressure uses logarithmic spacing, wind uses symmetric levels around zero), and returns an
        array of contour levels optimized for the specific atmospheric variable. It provides reasonable
        defaults when explicit levels are not specified.
    ...
    def _convert_vertical_to_height(self, 
        ...
        """
        Converts vertical coordinates to height in kilometers when possible using standard atmosphere
        approximations or geometric height variables. This internal method handles three coordinate
        types: direct height conversion by dividing by 1000, pressure-to-height conversion using
        standard atmosphere scale height formula (h = -H*ln(P/P0) where H=8.4km), and model level
        conversion by searching for geometric height variables (zgrid, height) in the dataset. It
        returns converted coordinates and an updated coordinate type string for axis labeling.
    ...
    def _format_cross_section_axes(self, 
        ...
        """
        Formats cross-section plot axes with appropriate labels, limits, and coordinate-specific settings
        including inverted pressure axes and logarithmic scaling. This internal method configures the
        x-axis for longitude with custom formatting, sets y-axis properties based on vertical coordinate
        type (height in km with upward orientation, pressure in hPa/Pa with inverted logarithmic scale,
        model levels with top-to-bottom ordering), applies optional maximum height limits with unit
        conversion, and adds a path information text box showing start and end coordinates.
    ...
    def _get_time_string(self, mpas_3d_processor: MPAS3DProcessor, time_index: int) -> str:
        """
        Retrieves and formats a time string from the 3D processor dataset for plot annotations using
        multiple fallback strategies. This internal method first attempts to use the processor's get_time_info
        method if available, then tries to extract and format timestamps from the dataset's Time coordinate
        using pandas datetime conversion with standard UTC format string, and falls back to displaying
        the time index number if no time information is accessible. It returns a human-readable time
        string suitable for plot titles and labels.
    ...
    def create_batch_cross_section_plots(self, 
                                       mpas_3d_processor: Any,
                                       output_dir: str,
                                       var_name: str,
                                       start_point: Tuple[float, float],
        ...
        """
        Creates vertical cross-section plots for all time steps in the dataset using batch processing
        with consistent styling and automatic file naming. This method iterates through all available
        time steps, extracts timestamps for filename generation, creates individual cross-section plots
        with uniform vertical extent and contour levels, saves each plot in multiple formats with
        descriptive filenames including vertical coordinate and time information, provides progress
        updates every 5 steps, and returns a complete list of created file paths.
