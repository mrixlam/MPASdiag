...
CONDENSATE_VARS = [DEFAULT_QC_VAR, DEFAULT_QR_VAR,
                   DEFAULT_QI_VAR, DEFAULT_QS_VAR, DEFAULT_QG_VAR]
...
class ThermodynamicDiagnostics:
    ...
    def __init__(self: 'ThermodynamicDiagnostics', 
    ...
    def compute_density_potential_temperature(self: 'ThermodynamicDiagnostics',
                                              theta: xr.DataArray,
                                              qv: xr.DataArray,
                                              ...
                                              qg: Optional[xr.DataArray] = None,) -> xr.DataArray:
        """
    ...
    def compute_density_potential_temperature_from_dataset(self: 'ThermodynamicDiagnostics',
                                                           dataset: xr.Dataset,
                                                           time_index: int = 0,
        ...
        time_dim, validated_time_index, _ = (
            MPASDateTimeUtils.validate_time_parameters(
        ...
        theta = self._extract_field(
            dataset, theta_var, time_dim, validated_time_index,
            level, data_type,
        )

        # Extract qv and check for missing variable
        qv = self._extract_field(
            dataset, qv_var, time_dim, validated_time_index,
            level, data_type,
        )
        ...
        for cvar in condensate_vars:
            if cvar in dataset.data_vars:
                condensates[cvar] = self._extract_field(
                    dataset, cvar, time_dim, validated_time_index,
                    level, data_type,
                )
            elif self.verbose:
                logger.debug("Condensate variable '%s' not in dataset — treating as zero", cvar)
    ...
    def compute_virtual_potential_temperature(self: 'ThermodynamicDiagnostics',
                                              theta: xr.DataArray,
                                              qv: xr.DataArray,) -> xr.DataArray:
        """
        ...
        if theta is None or qv is None:
            raise ValueError(
                "Both 'theta' and 'qv' must be provided."
            )
        ...
        if self.verbose:
            ...
            logger.debug(
                ...
                tv_min, tv_max, tv_mean,
            )

        # Return the computed virtual potential temperature
        return theta_v

    def compute_total_water_mixing_ratio(self: 'ThermodynamicDiagnostics',
                                         qv: xr.DataArray,
                                         qc: Optional[xr.DataArray] = None,
                                         qr: Optional[xr.DataArray] = None,
                                         qi: Optional[xr.DataArray] = None,
        ...
        for name, species in [('qc', qc), ('qr', qr),
                              ('qi', qi), ('qs', qs), ('qg', qg)]:
    ...
    def analyze_density_potential_temperature(self: 'ThermodynamicDiagnostics',
                                              theta_rho: xr.DataArray,
                                              theta: Optional[xr.DataArray] = None,) -> Dict[str, Any]:
        """
    ...
    def compute_cold_pool_strength(self: 'ThermodynamicDiagnostics',
                                   theta_rho: xr.DataArray,
                                   theta_rho_env: Optional[Union[float, xr.DataArray]] = None,
                                   cold_pool_depth: float = 1000.0,
                                   g: float = G,) -> Tuple[xr.DataArray, xr.DataArray]:
        """
    ...
    def _extract_field(self: 'ThermodynamicDiagnostics',
                       dataset: xr.Dataset,
                       var_name: str,
                       time_dim: str,
                       time_index: int,
        ...
        if data_type == 'uxarray' and hasattr(dataset, '__getitem__'):
            ...
            if type(dataset) is not xr.Dataset and isinstance(dataset, xr.Dataset):
                plain_ds = xr.Dataset(dict(dataset.data_vars), coords=dataset.coords, attrs=dataset.attrs)
            else:
                plain_ds = dataset
    ...
    def _resolve_level_index(self: 'ThermodynamicDiagnostics',
                             dataset: xr.Dataset,
                             var_name: str,
        ...
        if isinstance(level, float):
            return self._pressure_to_level_index(
                dataset, level, time_dim, time_index
            )
    ...
    def _pressure_to_level_index(self: 'ThermodynamicDiagnostics',
                                 dataset: xr.Dataset,
                                 pressure_pa: float,
                                 time_dim: str,
                                 time_index: int,) -> int:
        ...
        if self.verbose:
            ...
            logger.debug(
                ...
                pressure_pa, level_idx, actual_p,
            )
    ...
    def _print_input_summary(self: 'ThermodynamicDiagnostics',
                             theta: xr.DataArray,
                             qv: xr.DataArray,
                             qt: xr.DataArray,) -> None:
        """
        ...
        if is_3d:
            logger.debug(
                ...
                float(theta.min()), float(theta.max()),
            )
            sfc = theta.isel({vdim: 0})
            logger.debug(
                "  theta range (surface lev) : %.2f to %.2f K",
                float(sfc.min()), float(sfc.max()),
            )
        else:
            logger.debug(
    ...
    def _print_result_summary(self: 'ThermodynamicDiagnostics', 
                              theta_rho: xr.DataArray) -> None:
        """
        ...
        if is_3d:
            ...
            logger.debug(
                ...
                float(theta_rho.min()), float(theta_rho.max()),
            )
