...
def _trapezoidal_column_integral(
    integrand_arr: np.ndarray, pressure_arr: np.ndarray
) -> np.ndarray:
    """
    ...
    column_integral: np.ndarray = (
        np.sum(integrand_midpoints * pressure_thickness, axis=-1) / GRAVITY
    )
...
class MoistureTransportDiagnostics:
    ...
    def __init__(self: "MoistureTransportDiagnostics", verbose: bool = True) -> None:
        """
    ...
    def _integrate_column(
        self: "MoistureTransportDiagnostics",
    ...
    def compute_ivt_components(
        ...
        if self.verbose:
            logger.debug(
                "IVT_u range: %.2f to %.2f kg/(m·s)",
                float(ivt_u.min()),
                float(ivt_u.max()),
            )
            logger.debug("IVT_u mean:  %.2f kg/(m·s)", float(ivt_u.mean()))
            logger.debug(
                "IVT_v range: %.2f to %.2f kg/(m·s)",
                float(ivt_v.min()),
                float(ivt_v.max()),
            )
    ...
    def compute_ivt(
        self: "MoistureTransportDiagnostics", ivt_u: xr.DataArray, ivt_v: xr.DataArray
    ) -> xr.DataArray:
        """
        ...
        ivt = xr.apply_ufunc(
            np.sqrt, ivt_u**2 + ivt_v**2, keep_attrs=False, dask="parallelized"
        )
    ...
    def analyze_moisture_transport(
        self: "MoistureTransportDiagnostics",
        specific_humidity: xr.DataArray,
        u_component: xr.DataArray,
        v_component: xr.DataArray,
        ...
        ivt_u, ivt_v = self.compute_ivt_components(
