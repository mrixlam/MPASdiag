...
def synthetic_3d_dataset() -> xr.Dataset:
    ...
    theta_3d = np.tile(theta, (n_time, n_cells, 1)) + rng.normal(
        0, 1, (n_time, n_cells, n_vert)
    )

    qv = np.linspace(0.015, 0.0001, n_vert)
    qv_3d = np.tile(qv, (n_time, n_cells, 1)) * rng.uniform(
        0.8, 1.2, (n_time, n_cells, n_vert)
    )
...
@pytest.fixture
def mock_processor(synthetic_3d_dataset: xr.Dataset, tmp_path: Path) -> Mock:
    """
...
class TestThermodynamicIndices:
    ...
    def test_returns_dict_keys(
        self: "TestThermodynamicIndices", diag: "SoundingDiagnostics"
    ) -> None:
        """
    ...
    def test_wind_indices_none_without_wind(
        self: "TestThermodynamicIndices", diag: "SoundingDiagnostics"
    ) -> None:
        """
...
class TestExtractSoundingProfile:
    ...
    def test_basic_extraction(
        self: "TestExtractSoundingProfile",
        ...
        profile = diag.extract_sounding_profile(
            mock_processor, -100.0, 35.0, time_index=0
        )
    ...
    def test_pressure_sorted_descending(
        self: "TestExtractSoundingProfile",
        diag: "SoundingDiagnostics",
        mock_processor: "MPAS3DProcessor",
    ) -> None:
    ...
    def test_wind_components_present(
        self: "TestExtractSoundingProfile",
        diag: "SoundingDiagnostics",
        mock_processor: "MPAS3DProcessor",
    ) -> None:
    ...
    def test_different_time_indices(
        self: "TestExtractSoundingProfile",
        diag: "SoundingDiagnostics",
        mock_processor: "MPAS3DProcessor",
    ) -> None:
...
class TestEdgeCases:
    ...
    def test_missing_moisture_variable(
        self: "TestEdgeCases",
        diag: "SoundingDiagnostics",
        tmp_path: "Path",
        mock_processor: "MPAS3DProcessor",
        ...
        ds = xr.Dataset(
            {
                "pressure": (
                    ...
                    np.linspace(101000, 5000, n_vert)
                    .reshape(1, 1, n_vert)
                    .repeat(n_cells, axis=1),
                ),
                "theta": (
                    ["Time", "nCells", "nVertLevels"],
    ...
    def test_missing_wind_variables(
        self: "TestEdgeCases",
        diag: "SoundingDiagnostics",
        tmp_path: "Path",
        mock_processor: "MPAS3DProcessor",
