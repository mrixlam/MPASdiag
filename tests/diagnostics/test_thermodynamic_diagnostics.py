...
def _make_3d_dataset(n_time: int = 2,
                     n_cells: int = 6,
                     n_vert: int = 4,
                     include_condensates: bool = True,
                     include_pressure: bool = True,
                     vert_name: str = 'nVertLevels',) -> xr.Dataset:
    """
    ...
    data_vars = {
        'theta': xr.DataArray(
            theta_vals, dims=['Time', 'nCells', vert_name],
            coords={'Time': times},
        ),
        'qv': xr.DataArray(
            qv_vals, dims=['Time', 'nCells', vert_name],
            coords={'Time': times},
        ),
    }
    ...
    if include_pressure:
        ...
        data_vars['pressure_base'] = xr.DataArray(
            pb, dims=['Time', 'nCells', vert_name],
        )
        data_vars['pressure_p'] = xr.DataArray(
            pp, dims=['Time', 'nCells', vert_name],
        )
...
def _make_2d_dataset() -> xr.Dataset:
    ...
    return xr.Dataset({
        'theta': xr.DataArray(
            np.full((2, n_cells), 300.0), dims=['Time', 'nCells'],
            coords={'Time': times},
        ),
        'qv': xr.DataArray(
            np.full((2, n_cells), 0.01), dims=['Time', 'nCells'],
            coords={'Time': times},
        ),
    })
...
class TestComputeDensityPotentialTemperature:
    ...
    def test_raises_when_theta_none(self: 'TestComputeDensityPotentialTemperature',
                                    diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_raises_when_qv_none(self: 'TestComputeDensityPotentialTemperature',
                                 diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_computes_correct_values_without_condensates(self: 'TestComputeDensityPotentialTemperature', 
                                                         diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_computes_with_explicit_qt(self: 'TestComputeDensityPotentialTemperature', 
                                       diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_computes_with_individual_condensates(self: 'TestComputeDensityPotentialTemperature', 
                                                  diag_silent: ThermodynamicDiagnostics,) -> None:
        """
        ...
        result = diag_silent.compute_density_potential_temperature(
            theta, qv, qc=qc, qr=qr, qi=qi, qs=qs, qg=qg,
        )
        qt_expected = 0.01 + 0.0005 + 0.0001 + 0.00005 + 0.00002 + 0.00001
        expected = 300.0 * (1.0 + RV_OVER_RD * 0.01 - qt_expected)
        assert np.allclose(result.values, expected)

    def test_verbose_runs_print_helpers_2d(self: 'TestComputeDensityPotentialTemperature', 
    ...
    def test_verbose_runs_print_helpers_3d(self: 'TestComputeDensityPotentialTemperature', 
                                           diag_verbose: ThermodynamicDiagnostics,) -> None:
        """
        ...
        theta = xr.DataArray(
            np.full((4, 3), 300.0), dims=['nCells', 'nVertLevels'],
        )
        qv = xr.DataArray(
            np.full((4, 3), 0.01), dims=['nCells', 'nVertLevels'],
        )
        result = diag_verbose.compute_density_potential_temperature(theta, qv)
...
class TestComputeFromDataset:
    ...
    def test_raises_when_dataset_none(self: 'TestComputeFromDataset', 
                                      diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_full_3d_without_level(self: 'TestComputeFromDataset', 
    ...
    def test_with_int_level(self: 'TestComputeFromDataset', 
                            diag_silent: ThermodynamicDiagnostics, 
                            ds_3d: xr.Dataset,) -> None:
        """
        ...
        """
        result = diag_silent.compute_density_potential_temperature_from_dataset(
            ds_3d, time_index=0, level=1,
        )
        assert 'nVertLevels' not in result.dims

    def test_with_surface_level(self: 'TestComputeFromDataset', 
                                diag_silent: ThermodynamicDiagnostics, 
                                ds_3d: xr.Dataset,) -> None:
        """
        ...
        """
        result = diag_silent.compute_density_potential_temperature_from_dataset(
            ds_3d, time_index=0, level='surface',
        )
        assert 'nVertLevels' not in result.dims

    def test_with_top_level(self: 'TestComputeFromDataset', 
                            diag_silent: ThermodynamicDiagnostics, 
                            ds_3d: xr.Dataset,) -> None:
        """
        ...
        """
        result = diag_silent.compute_density_potential_temperature_from_dataset(
            ds_3d, time_index=0, level='top',
        )
        assert 'nVertLevels' not in result.dims

    def test_with_pressure_level(self: 'TestComputeFromDataset', 
        ...
        result = diag_verbose.compute_density_potential_temperature_from_dataset(
            ds_3d, time_index=0, level=85000.0,
        )
        assert 'nVertLevels' not in result.dims

    def test_missing_var_raises(self: 'TestComputeFromDataset', 
                                diag_silent: ThermodynamicDiagnostics, 
                                ds_3d: xr.Dataset,) -> None:
        """
        ...
        with pytest.raises(ValueError, match="not found in dataset"):
            diag_silent.compute_density_potential_temperature_from_dataset(
                ds_3d, theta_var='nope',
            )

    def test_missing_condensates_treated_as_zero(self: 'TestComputeFromDataset', 
                                                 diag_verbose: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_time_index_clamped_when_out_of_range(self: 'TestComputeFromDataset', 
                                                  diag_silent: ThermodynamicDiagnostics, 
                                                  ds_3d: xr.Dataset,) -> None:
        """
    ...
    def test_uxarray_data_type_branch(self: 'TestComputeFromDataset', 
                                      diag_silent: ThermodynamicDiagnostics,) -> None:
        """
        ...
        ds = _make_3d_dataset(include_condensates=False, include_pressure=False)
        result = diag_silent.compute_density_potential_temperature_from_dataset(
            ds, time_index=0, data_type='uxarray',
        )
        assert result is not None

    def test_uxarray_subclass_dataset_unwrapped(self: 'TestComputeFromDataset', 
                                                diag_silent: ThermodynamicDiagnostics,) -> None:
        """
        ...
        ux_ds = _UxLikeDataset(
            dict(base.data_vars), coords=base.coords, attrs=base.attrs,
        )
        result = diag_silent.compute_density_potential_temperature_from_dataset(
            ux_ds, time_index=0, level=85000.0,
        )
        assert result is not None

    def test_explicit_condensate_vars_list(self: 'TestComputeFromDataset', 
                                           diag_silent: ThermodynamicDiagnostics, 
                                           ds_3d: xr.Dataset,) -> None:
        """
        ...
        result = diag_silent.compute_density_potential_temperature_from_dataset(
            ds_3d, time_index=0, condensate_vars=['qc', 'qr'],
        )
        assert result is not None
...
class TestVirtualPotentialTemperature:
    ...
    def test_raises_when_theta_or_qv_none(self: 'TestVirtualPotentialTemperature', 
                                          diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_computes_correct_values(self: 'TestVirtualPotentialTemperature', 
                                     diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_verbose_runs(self: 'TestVirtualPotentialTemperature',
                          diag_verbose: ThermodynamicDiagnostics) -> None:
        """
...
class TestTotalWaterMixingRatio:
    ...
    def test_only_qv_returns_copy(self: 'TestTotalWaterMixingRatio', 
                                  diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_sums_all_condensates(self: 'TestTotalWaterMixingRatio', 
                                  diag_verbose: ThermodynamicDiagnostics,) -> None:
        """
        ...
        result = diag_verbose.compute_total_water_mixing_ratio(
            qv, qc=qc, qr=qr, qi=qi, qs=qs, qg=qg,
        )
...
class TestAnalyze:
    ...
    def test_without_theta(self: 'TestAnalyze',
                           diag_silent: ThermodynamicDiagnostics) -> None:
        """
    ...
    def test_with_theta_adds_perturbation(self: 'TestAnalyze', 
                                          diag_verbose: ThermodynamicDiagnostics,) -> None:
        """
...
class TestColdPoolStrength:
    ...
    def test_with_explicit_float_env(self: 'TestColdPoolStrength', 
                                     diag_silent: ThermodynamicDiagnostics,) -> None:
        """
        ...
        c, b = diag_silent.compute_cold_pool_strength(
            theta_rho, theta_rho_env=300.0, cold_pool_depth=1000.0,
        )
    ...
    def test_env_none_uses_domain_mean(self: 'TestColdPoolStrength', 
                                       diag_verbose: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_env_dataarray(self: 'TestColdPoolStrength', 
                           diag_silent: ThermodynamicDiagnostics,) -> None:
        """
...
class TestResolveLevelIndex:
    ...
    def test_int_level_out_of_range_raises(self: 'TestResolveLevelIndex', 
                                           diag_silent: ThermodynamicDiagnostics, 
                                           ds_3d: xr.Dataset,) -> None:
        """
    ...
    def test_unknown_string_level_raises(self: 'TestResolveLevelIndex', 
                                         diag_silent: ThermodynamicDiagnostics, 
                                         ds_3d: xr.Dataset,) -> None:
        """
    ...
    def test_invalid_level_type_raises(self: 'TestResolveLevelIndex', 
                                       diag_silent: ThermodynamicDiagnostics, 
                                       ds_3d: xr.Dataset,) -> None:
        """
        ...
        with pytest.raises(ValueError, match="Invalid level specification"):
            diag_silent.compute_density_potential_temperature_from_dataset(
                ds_3d, level=[0, 1],  # type: ignore[arg-type]
            )

    def test_pressure_level_without_pressure_vars_raises(self: 'TestResolveLevelIndex', 
                                                         diag_silent: ThermodynamicDiagnostics,) -> None:
        """
    ...
    def test_2d_data_with_level_skips_vertical_selection(self: 'TestResolveLevelIndex', 
                                                         diag_silent: ThermodynamicDiagnostics, 
    ...
    def test_nvertlevelsp1_dim_handled(self: 'TestResolveLevelIndex', 
                                       diag_silent: ThermodynamicDiagnostics,) -> None:
        """
        ...
        """
        ds = _make_3d_dataset(
            include_condensates=False, vert_name='nVertLevelsP1',
        )
        result = diag_silent.compute_density_potential_temperature_from_dataset(
            ds, time_index=0, level=70000.0,
        )
...
class TestStaticHelpers:
    ...
    def test_resolve_vertical_dim_finds_nvertlevelsp1(self: 'TestStaticHelpers') -> None:
        """
