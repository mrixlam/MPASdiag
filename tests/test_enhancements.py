...
class TestConditionalTimeDisplay:
    ...
    def setup_method(
        self: "TestConditionalTimeDisplay", mpas_surface_temp_data: np.ndarray
    ...
    def _setup_mocks(
        self: "TestConditionalTimeDisplay",
    ) -> Tuple[MagicMock, MagicMock, List[Tuple[Tuple, dict]]]:
        """
        ...
        def capture_text(*args: Any, **kwargs: Any) -> None:
            """
    ...
    def test_default_title_with_time(self: "TestConditionalTimeDisplay") -> None:
        ...
        corner_text_calls = [
            ...
            if len(call[0]) >= 3
            and math.isclose(call[0][1], 0.98, abs_tol=1e-6)
            and math.isclose(call[0][2], 0.02, abs_tol=1e-6)
        ]

        assert len(corner_text_calls) == pytest.approx(
            0
        ), "Corner text should not be displayed when time is in title"
    ...
    def test_custom_title_without_time(self: "TestConditionalTimeDisplay") -> None:
        ...
        assert (
            len(corner_text_calls) > 0
        ), "Corner text should be displayed when time is not in title"
    ...
    def test_custom_title_with_time(self: "TestConditionalTimeDisplay") -> None:
        ...
        assert len(corner_text_calls) == pytest.approx(
            0
        ), "Corner text should not be displayed when time is already in title"
    ...
    def test_no_timestamp(self: "TestConditionalTimeDisplay") -> None:
        ...
        assert len(corner_text_calls) == pytest.approx(
            0
        ), "No time display should appear when timestamp is None"
