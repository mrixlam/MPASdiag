...
class TestHelperMethodsAndOutput:
    ...
    def setup_method(
        self: "TestHelperMethodsAndOutput", mock_mpas_mesh: Any, mock_mpas_2d_data: Any
        ...
        combined_ds = xr.Dataset({"t2m": self.mock_mpas_2d_data["t2m"].isel(Time=0)})
...
class TestDataLoadingStrategies:
    ...
    @pytest.fixture(autouse=True)
    def setup_method(
        self: "TestDataLoadingStrategies", mock_mpas_mesh: Any, mock_mpas_2d_data: Any
    ) -> Generator[None, None, None]:
        """
...
class TestEdgeCasesAndErrorHandling:
    ...
    @pytest.fixture(autouse=True)
    def setup_method(
        self: "TestEdgeCasesAndErrorHandling", mock_mpas_mesh: Any
    ) -> Generator[None, None, None]:
        """
...
class TestVisualizerOperations:
    ...
    def setup_method(self: "TestVisualizerOperations", mock_mpas_3d_data: Any) -> None:
        """
    ...
    def test_create_time_series_plot_numpy_datetime(
        self: "TestVisualizerOperations",
    ) -> None:
        """
    ...
    def test_create_time_series_custom_labels(self: "TestVisualizerOperations") -> None:
        ...
        _, ax = self.visualizer.create_time_series_plot(
            times, values, title="Custom Title", ylabel="Custom Y", xlabel="Custom X"
        )
...
class TestVisualizerHelperMethods:
    ...
    def setup_visualizer(
        self: "TestVisualizerHelperMethods",
    ) -> Generator[None, None, None]:
        """
    ...
    def test_create_time_series_plot_with_non_datetime_times(
        self: "TestVisualizerHelperMethods",
    ) -> None:
        """
        ...
        fig, ax = self.viz.create_time_series_plot(
            times, values, title="T", xlabel="X", ylabel="Y"
        )
        assert fig is not None
        assert ax is not None

    def test_create_histogram_bins_as_ndarray(
        self: "TestVisualizerHelperMethods",
    ) -> None:
        """
...
class TestTransectOverlay:
    ...
    def test_draw_transect_line_returns_all_artist_keys(
        self: "TestTransectOverlay",
    ) -> None:
        """
    ...
    def test_draw_transect_lines_returns_all_labels(
        self: "TestTransectOverlay",
    ) -> None:
        """
    ...
    def test_draw_transect_lines_per_transect_color_override(
        self: "TestTransectOverlay",
    ) -> None:
        """
        ...
        result = self.viz.draw_transect_lines(
            ax, transects, style=TransectLineStyle(color="red")
        )
        line_ab = result["A-B"]["line"][0]
        line_cd = result["C-D"]["line"][0]
        assert np.allclose(
            to_rgba(line_ab.get_color()), to_rgba("royalblue"), atol=1e-3
        )
        assert np.allclose(to_rgba(line_cd.get_color()), to_rgba("red"), atol=1e-3)
        plt.close(fig)

    def test_draw_transect_lines_per_transect_linewidth_override(
        self: "TestTransectOverlay",
    ) -> None:
        """
    ...
    def test_draw_transect_lines_show_labels_false(self: "TestTransectOverlay") -> None:
        ...
        result = self.viz.draw_transect_lines(
            ax, transects, style=TransectLineStyle(show_labels=False)
        )
