...
class TestEnhancedSurfacePlotting:
    ...
    def setup_method(
        self: "TestEnhancedSurfacePlotting",
    ...
    def test_extract_2d_from_3d_with_index(self: "TestEnhancedSurfacePlotting") -> None:
        ...
        temp_base = 220.0 + 80.0 * (
            (u_arr - u_arr.min()) / (u_arr.max() - u_arr.min() + 1e-12)
        )
        data_3d = np.tile(
            temp_base.reshape((self.n_cells, 1, 1)), (1, self.n_levels, 1)
        )
    ...
    def test_extract_2d_from_3d_with_coordinate_value(
        self: "TestEnhancedSurfacePlotting",
    ) -> None:
        """
        ...
        result = self.visualizer.extract_2d_from_3d(
            data_xr, level_value=850, level_dim="pressure"
        )
    ...
    def test_extract_2d_from_3d_error_cases(
        self: "TestEnhancedSurfacePlotting",
    ) -> None:
        """
    ...
    def test_create_surface_map_3d_data_with_level_index(
        self: "TestEnhancedSurfacePlotting",
    ) -> None:
        """
    ...
    def test_create_surface_map_2d_multilevel_data(
        self: "TestEnhancedSurfacePlotting",
    ) -> None:
        """
        ...
        _, _, _, _ = load_mpas_coords_from_processor(n=self.n_cells)
        temp_levels = np.tile(
            self.temp_data[: self.n_cells].reshape((self.n_cells, 1)),
            (1, self.n_levels),
        )
    ...
    def test_wind_overlay_2d_data(self: "TestEnhancedSurfacePlotting") -> None:
        ...
        fig, ax = self.visualizer.create_surface_map(
            ...
            style=SurfaceMapStyle(
                title="Test Temperature with 2D Wind Overlay", wind_overlay=wind_config
            ),
        )
    ...
    def test_wind_overlay_3d_data(self: "TestEnhancedSurfacePlotting") -> None:
        ...
        temp_2d = self.temp_data[: self.n_cells]
        u_wind_3d = np.tile(
            self.u[: self.n_cells].reshape((self.n_cells, 1)), (1, self.n_levels)
        )
        v_wind_3d = np.tile(
            self.v[: self.n_cells].reshape((self.n_cells, 1)), (1, self.n_levels)
        )
        ...
        fig, ax = self.visualizer.create_surface_map(
            ...
            GeographicBounds(self.lon_min, self.lon_max, self.lat_min, self.lat_max),
            style=SurfaceMapStyle(
                title="Test Temperature with 3D Wind Overlay", wind_overlay=wind_config
            ),
        )
    ...
    def test_wind_overlay_arrows(self: "TestEnhancedSurfacePlotting") -> None:
        ...
        fig, _ = self.visualizer.create_surface_map(
            ...
            GeographicBounds(self.lon_min, self.lon_max, self.lat_min, self.lat_max),
            style=SurfaceMapStyle(
                title="Test Temperature with Wind Arrows", wind_overlay=wind_config
            ),
        )
    ...
    def test_auto_subsampling(self: "TestEnhancedSurfacePlotting") -> None:
        ...
        temp_large = np.tile(
            temp_base_large, int(np.ceil(n_large / len(temp_base_large)))
        )[:n_large]
        ...
        fig, _ = self.visualizer.create_surface_map(
            ...
            GeographicBounds(self.lon_min, self.lon_max, self.lat_min, self.lat_max),
            style=SurfaceMapStyle(
                title="Test Auto-Subsampling", wind_overlay=wind_config
            ),
        )
    ...
    def test_custom_colormap_and_levels(self: "TestEnhancedSurfacePlotting") -> None:
        ...
        fig, _ = self.visualizer.create_surface_map(
            ...
            GeographicBounds(self.lon_min, self.lon_max, self.lat_min, self.lat_max),
            style=SurfaceMapStyle(
                plot_type="contour", colormap="coolwarm", levels=custom_levels
            ),
        )
...
class TestDataTypeAgnosticFeatures:
    ...
    @pytest.fixture(autouse=True)
    def setup_method(
        self: "TestDataTypeAgnosticFeatures",
        mpas_surface_temp_data: np.ndarray,
        mpas_wind_data: tuple[np.ndarray, np.ndarray],
        mpas_precip_data: np.ndarray,
    ) -> Iterator[None]:
    ...
    def test_mixed_data_types_wind_overlay(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
        ...
        temp_xr = xr.DataArray(self.temp_data[: self.n_cells], dims=["cells"])
    ...
    def test_surface_overlay_contour_lines(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
        ...
        geop_2d = 1200.0 + 200.0 * (
            self.precip_data[: self.n_cells]
            / (np.max(self.precip_data[: self.n_cells]) + 1e-12)
        )
    ...
    def test_surface_overlay_filled_contours(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
    ...
    def test_complete_850hpa_weather_map(self: "TestDataTypeAgnosticFeatures") -> None:
        ...
        temp_850 = 263.0 + 30.0 * (
            self.temp_data[: self.n_cells]
            / (np.max(self.temp_data[: self.n_cells]) + 1e-12)
        )
        geop_850 = 1200.0 + 400.0 * (
            self.precip_data[: self.n_cells]
            / (np.max(self.precip_data[: self.n_cells]) + 1e-12)
        )
    ...
    def test_surface_overlay_error_handling(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
    ...
    def test_multiple_overlays_interaction(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
    ...
    def test_variable_specific_temperature_settings(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
        ...
        colormap, levels = self.visualizer.get_variable_specific_settings(
            "temperature", temp_k
        )
    ...
    def test_variable_specific_precipitation_settings(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
    ...
    def test_variable_specific_pressure_settings(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
    ...
    def test_variable_specific_wind_settings(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
        ...
        wind_data = 20.0 * (
            np.hypot(u_arr, v_arr) / (np.hypot(u_arr, v_arr).max() + 1e-12)
        )
        colormap, levels = self.visualizer.get_variable_specific_settings(
    ...
    def test_variable_specific_geopotential_settings(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
        ...
        geop_data = 1200.0 + 400.0 * (
            self.precip_data[: self.n_cells]
            / (np.max(self.precip_data[: self.n_cells]) + 1e-12)
        )
        colormap, levels = self.visualizer.get_variable_specific_settings(
            "geopotential_height", geop_data
        )
    ...
    def test_variable_specific_humidity_settings(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
    ...
    def test_variable_specific_unknown_variable(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
        ...
        _, _, u_arr, _ = load_mpas_coords_from_processor(n=self.n_cells)
        diverging_data = (
            (u_arr - u_arr.mean()) / (u_arr.max() - u_arr.min() + 1e-12) * 100.0
        )
        colormap, levels = self.visualizer.get_variable_specific_settings(
            "unknown_var", diverging_data
        )

        assert colormap == "RdBu_r"
        assert isinstance(levels, list)

        sequential_data = 100.0 * (
            (u_arr - u_arr.min()) / (u_arr.max() - u_arr.min() + 1e-12)
        )
        colormap, levels = self.visualizer.get_variable_specific_settings(
            "unknown_positive", sequential_data
        )

        assert colormap == "viridis"
        assert isinstance(levels, list)

    def test_variable_specific_integration_with_plotting(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
        ...
        fig, _ = self.visualizer.create_surface_map(
            ...
            GeographicBounds(self.lon_min, self.lon_max, self.lat_min, self.lat_max),
            style=SurfaceMapStyle(
                title="Temperature with Auto Settings", plot_type="contourf"
            ),
        )
    ...
    def test_variable_specific_with_surface_overlays(
        self: "TestDataTypeAgnosticFeatures",
    ) -> None:
        """
