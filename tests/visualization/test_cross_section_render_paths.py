...
class TestCrossSectionRenderingPaths:
    ...
    def test_pressure_nonpositive_warning(
        self: "TestCrossSectionRenderingPaths", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
...
class TestCrossSectionAxisFormatting:
    ...
    def test_pressure_to_height_conversion_standard_atmosphere(
        self: "TestCrossSectionAxisFormatting",
    ) -> None:
        """
...
class TestCrossSectionBatch:
    ...
    def test_batch_cross_section_time_iteration(
        self: "TestCrossSectionBatch", tmp_path: str
    ) -> None:
        """
        ...
        mock_proc.dataset = xr.Dataset(
            {
                "theta": (
                    ...
                    _RNG.uniform(200, 350, (3, 5, 100)),
                ),
                "Time": (
                    ["Time"],
                    pd.date_range("2025-01-01", periods=3, freq="h").values,
                ),
            }
        )

        with (
            patch.object(plotter, "create_vertical_cross_section") as mock_create,
