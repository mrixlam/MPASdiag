...
class TestPlotTypes:
    ...
    def test_plot_with_clim_and_levels(self: "TestPlotTypes") -> None:
        ...
        fig, _ = self.plotter.create_surface_map(
            ...
            style=SurfaceMapStyle(
                plot_type="contourf", levels=levels, clim_min=260, clim_max=290
            ),
        )
    ...
    def test_plot_with_clim_min_not_in_levels(self: "TestPlotTypes") -> None:
        ...
        fig, _ = self.plotter.create_surface_map(
            ...
            GeographicBounds(*self.extent_bounds),
            style=SurfaceMapStyle(
                plot_type="contourf", levels=levels, clim_min=260, clim_max=295
            ),
        )
...
class TestColorbarMethod:
    ...
    def test_colorbar_with_metadata(
        self: "TestColorbarMethod",
        mpas_coordinates: tuple,
        mpas_surface_temp_data: np.ndarray,
    ) -> None:
        """
...
class TestContourPlotting:
    ...
    def test_contour_label_exception(self: "TestContourPlotting") -> None:
        ...
        """
        with patch(
            "matplotlib.axes.Axes.clabel", side_effect=Exception("Labeling failed")
        ):
            fig, _ = self.plotter.create_surface_map(
                self.lon,
                self.lat,
