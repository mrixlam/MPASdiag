...
def _simple_lon_lat() -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    return (
        np.linspace(-100.0, -90.0, N_CELLS),
...
class TestAddRegionalFeaturesAxNone:
    ...
    def test_no_ax_returns_silently(self: "TestAddRegionalFeaturesAxNone") -> None:
        ...
        v = _viz()
        assert v.ax is None
        v.add_regional_features(-100.0, -90.0, 35.0, 45.0)


class TestCreateTimeSeriesPlot:
    """Covers the pd.to_datetime fallback."""

    def test_datetime64_fallback_when_pd_fails(
        self: "TestCreateTimeSeriesPlot",
    ) -> None:
        """
        ...
        with patch(
            "mpasdiag.visualization.base_visualizer.pd.to_datetime",
    ...
    def test_normal_datetimes_work(self: "TestCreateTimeSeriesPlot") -> None:
        ...
        fig, _ = v.create_time_series_plot(times, [1.0, 2.0, 3.0])
        assert fig is not None
        plt.close(fig)
...
class TestCreateHistogram:
    ...
    def test_bins_tolist_exception_falls_back_to_list(
        self: "TestCreateHistogram",
    ) -> None:
        """
        ...
        with patch(
            "mpasdiag.visualization.base_visualizer.isinstance", wraps=isinstance
        ) as mock_isinstance:
            original_isinstance = isinstance

            def patched_isinstance(obj: Any, cls: Any) -> bool:
                """
...
class TestEnsureBoundaryData:
    ...
    def test_already_has_lon_b_lat_b_returns_unchanged(
        self: "TestEnsureBoundaryData",
    ) -> None:
        """
    ...
    def test_missing_mesh_vars_returns_unchanged(
        self: "TestEnsureBoundaryData",
    ) -> None:
        """
    ...
    def test_computes_boundary_from_mesh_topology(
        self: "TestEnsureBoundaryData",
    ) -> None:
        """
...
class TestExtractFullGrid:
        ...
        ds = xr.Dataset(
            {
                "lonCell": (
                    ["nCells"],
                    np.radians(np.linspace(-100.0, -90.0, N_CELLS)),
                ),
                "latCell": (["nCells"], np.radians(np.linspace(35.0, 45.0, N_CELLS))),
            }
        )
    ...
    def test_longitude_latitude_aliases(self: "TestExtractFullGrid") -> None:
        ...
        ds = xr.Dataset(
            {
                "longitude": (["nCells"], np.linspace(-100.0, -90.0, N_CELLS)),
                "latitude": (["nCells"], np.linspace(35.0, 45.0, N_CELLS)),
            }
        )
    ...
    def test_lon_lat_short_keys(self: "TestExtractFullGrid") -> None:
        ...
        ds = xr.Dataset(
            {
                "lon": (["nCells"], np.linspace(-100.0, -90.0, N_CELLS)),
                "lat": (["nCells"], np.linspace(35.0, 45.0, N_CELLS)),
            }
        )
        lon, _ = MPASVisualizer._extract_full_grid(ds)
...
class TestBackmapToFullGrid:
    ...
    def test_known_valid_points_mapped(self: "TestBackmapToFullGrid") -> None:
        ...
        Returns:
            None
        """
        lon_valid = np.array([-98.0, -95.0, -92.0])
        lat_valid = np.array([38.0, 40.0, 42.0])
        data_valid = np.array([10.0, 20.0, 30.0])
        lon_full = np.array([-98.0, -95.0, -92.0, -89.0])
        lat_full = np.array([38.0, 40.0, 42.0, 44.0])
    ...
    def test_distant_points_get_nan(self: "TestBackmapToFullGrid") -> None:
        ...
        Returns:
            None
        """
        lon_valid = np.array([-98.0])
        lat_valid = np.array([38.0])
        data_valid = np.array([10.0])
        lon_full = np.array([-50.0, -98.0])
        lat_full = np.array([10.0, 38.0])
...
class TestGetOrBuildRemapper:
    ...
    def test_builds_new_remapper_when_none_cached(
        self: "TestGetOrBuildRemapper",
    ) -> None:
        """
        ...
        """
        v = _viz()
        lon_full = np.linspace(-100.0, -90.0, N_CELLS)
        lat_full = np.linspace(35.0, 45.0, N_CELLS)
        lon_b = np.ones((N_CELLS, 3))
        lat_b = np.ones((N_CELLS, 3))

        mock_remapper = MagicMock()
        with patch(
            "mpasdiag.visualization.base_visualizer.MPASRemapper",
    ...
    def test_returns_cached_remapper_when_key_matches(
        self: "TestGetOrBuildRemapper",
    ) -> None:
        """
    ...
    def test_rebuilds_remapper_when_key_changes(self: "TestGetOrBuildRemapper") -> None:
        ...
        with patch(
            ...
            r1 = v._get_or_build_remapper(
                lon_full, lat_full, lon_b, lat_b, -100.0, -90.0, 35.0, 45.0, 1.0
            )
            r2 = v._get_or_build_remapper(
                lon_full, lat_full, lon_b, lat_b, -100.0, -90.0, 35.0, 45.0, 0.5
            )
        assert r1 is not r2
...
class TestRemapConservative:
    ...
    def test_calls_remapper_remap(self: "TestRemapConservative") -> None:
        ...
        ds = xr.Dataset(
            {
                "lon_b": (["nCells", "nv"], np.ones((N_CELLS, 3)) * -95.0),
                "lat_b": (["nCells", "nv"], np.ones((N_CELLS, 3)) * 40.0),
            }
        )
        lon_full = np.linspace(-100.0, -90.0, N_CELLS)
        lat_full = np.linspace(35.0, 45.0, N_CELLS)
        ...
        mock_remapper = MagicMock()
        lon_g = np.linspace(-100.0, -90.0, 5)
        lat_g = np.linspace(35.0, 45.0, 5)
...
class TestInterpolateToGridESMPy:
        ...
        with patch("mpasdiag.visualization.base_visualizer.ESMPY_AVAILABLE", True):
            with patch.object(v, "_ensure_boundary_data", return_value=ds):
                with patch.object(v, "_has_boundary_data", return_value=True):
                    with patch.object(v, "_extract_full_grid", return_value=(lon, lat)):
                        with patch.object(
                            v, "_backmap_to_full_grid", return_value=np.ones(N_CELLS)
                        ):
                            with patch.object(
                                v, "_remap_conservative", return_value=remap_result
                            ):
                    ...
                    with patch.object(v, "_extract_full_grid", return_value=(lon, lat)):
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                with patch(
                                    "mpasdiag.visualization.base_visualizer"
    ...
    def test_esmpy_engine_explicit_no_boundary_raises(
        self: "TestInterpolateToGridESMPy",
    ) -> None:
        """
...
class TestCreateContourPlot:
    ...
    def test_levels_none_else_branch(self: "TestCreateContourPlot") -> None:
        ...
        try:
            with patch.object(
                v, "_interpolate_to_grid", return_value=(lon_mesh, lat_mesh, data_2d)
            ):
    ...
    def test_contour_exception_raises_runtime_error(
        self: "TestCreateContourPlot",
    ) -> None:
        """
...
class TestCreateWindPlot:
    ...
    def test_auto_subsampling_dense_data(self: "TestCreateWindPlot") -> None:
        ...
        v = _viz()
        n_pts = 500
        lon = np.random.default_rng(0).uniform(-100.0, -99.0, n_pts)
        lat = np.random.default_rng(1).uniform(39.0, 40.0, n_pts)
        u = np.ones(n_pts) * 5.0
        vw = np.ones(n_pts) * 3.0
        ...
        try:
            fig, _ = v.create_wind_plot(
                ...
                u,
                vw,
                GeographicBounds(-100.0, -99.0, 39.0, 40.0),
...
class TestInterpolateToGridEdgeCases:
    ...
    def test_negative_resolution_raises(self: "TestInterpolateToGridEdgeCases") -> None:
        ...
        with pytest.raises(ValueError, match="grid_resolution must be"):
            v._interpolate_to_grid(
                lon, lat, data, -100.0, -90.0, 35.0, 45.0, grid_resolution=-1.0
            )

    def test_dataset_none_creates_minimal_dataset(
        self: "TestInterpolateToGridEdgeCases",
    ) -> None:
        """
        ...
        v = _viz()
        lon, lat = _simple_lon_lat()
        data = np.random.default_rng(0).uniform(0.0, 1.0, N_CELLS)
        ...
        with patch(
            "mpasdiag.visualization.base_visualizer.remap_mpas_to_latlon_with_masking",
