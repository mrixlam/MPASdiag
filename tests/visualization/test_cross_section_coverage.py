    ...
    return xr.Dataset(
        {
            "temperature": (
                ...
                np.linspace(250.0, 300.0, N_TIME * N_CELLS * N_VERT).reshape(
                    N_TIME, N_CELLS, N_VERT
                ),
            ),
            "qv": (
                ["Time", "nCells", "nVertLevels"],
            ...
            "lonCell": (["nCells"], np.linspace(-100.0, -90.0, N_CELLS)),
            "latCell": (["nCells"], np.linspace(35.0, 45.0, N_CELLS)),
        }
    )
...
class TestValidateCrossSectionInputs:
    ...
    def test_invalid_processor_type_raises(
        self: "TestValidateCrossSectionInputs",
    ) -> None:
        """
    ...
    def test_available_vars_listed_in_error(
        self: "TestValidateCrossSectionInputs",
    ) -> None:
        """
...
class TestConvertAndClipData:
    ...
    def test_moisture_clipping_preserves_positives(
        self: "TestConvertAndClipData",
    ) -> None:
        """
...
class TestResolveVerticalDisplay:
    ...
    def test_astype_exception_uses_np_asarray_fallback(
        self: "TestResolveVerticalDisplay",
    ) -> None:
        """
    ...
    def test_unknown_desired_display_falls_through(
        self: "TestResolveVerticalDisplay",
    ) -> None:
        """
        ...
        mock_proc = _make_proc(ds)
        mock_proc.grid_file = None
        vertical_coords = np.linspace(50000.0, 100000.0, N_VERT)
...
class TestApplyMaxHeightFilter:
    ...
    def test_xarray_vertical_display_converted(
        self: "TestApplyMaxHeightFilter",
    ) -> None:
        """
        ...
        """
        plotter = _plotter()
        vd = xr.DataArray(np.linspace(0.0, 10.0, N_VERT), dims=["level"])
        dv = np.ones((N_VERT, N_CELLS))
        out_vd, _ = plotter._apply_max_height_filter(
            vd, "height_km", dv, max_height=5.0
        )
    ...
    def test_list_vertical_display_converted(self: "TestApplyMaxHeightFilter") -> None:
        ...
        """
        plotter = _plotter()
        vd = list(np.linspace(0.0, 10.0, N_VERT))
        dv = np.ones((N_VERT, N_CELLS))
        out_vd, _ = plotter._apply_max_height_filter(
            vd, "height_km", dv, max_height=5.0
        )
    ...
    def test_xarray_data_values_converted(self: "TestApplyMaxHeightFilter") -> None:
        ...
        """
        plotter = _plotter()
        vd = np.linspace(0.0, 10.0, N_VERT)
        dv = xr.DataArray(np.ones((N_VERT, N_CELLS)), dims=["level", "cell"])
        _, out_dv = plotter._apply_max_height_filter(
            vd, "height_km", dv, max_height=5.0
        )
    ...
    def test_all_levels_above_max_shows_warning(
        self: "TestApplyMaxHeightFilter", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
        ...
        """
        plotter = _plotter()
        vd = np.linspace(6.0, 15.0, N_VERT)
        dv = np.ones((N_VERT, N_CELLS))
        out_vd, _ = plotter._apply_max_height_filter(
            vd, "height_km", dv, max_height=5.0
        )
    ...
    def test_no_max_height_returns_unchanged(self: "TestApplyMaxHeightFilter") -> None:
        ...
        dv = np.ones((N_VERT, N_CELLS))
        out_vd, _ = plotter._apply_max_height_filter(
            vd, "height_km", dv, max_height=None
        )
...
class TestResolvePlotStyle:
    ...
    def test_both_provided_returns_early(self: "TestResolvePlotStyle") -> None:
        ...
        plotter = _plotter()
        data = np.ones((5, 5))
        cmap, levels = plotter._resolve_plot_style(
            "temperature", data, "plasma", np.array([1.0, 2.0, 3.0])
        )
        assert cmap == "plasma"
        np.testing.assert_array_equal(levels, [1.0, 2.0, 3.0])

    def test_style_levels_none_triggers_get_default(
        self: "TestResolvePlotStyle",
    ) -> None:
        """
        ...
        data = np.ones((5, 5))
        with patch(
            "mpasdiag.visualization.cross_section.MPASVisualizationStyle.get_variable_style",
...
class TestResolveVerticalLevels:
    ...
    def test_unknown_vertical_coord_becomes_modlev(
        self: "TestResolveVerticalLevels",
    ) -> None:
        """
        ...
        plotter = _plotter()
        mock_proc = _make_proc(_make_ds())
        mock_proc.get_vertical_levels.return_value = np.arange(
            1, N_VERT + 1, dtype=float
        )
        _, coord = plotter._resolve_vertical_levels(
            mock_proc, "temperature", "foobar", 0
        )
        assert coord == "modlev"
    ...
    def test_height_coord_becomes_pressure(self: "TestResolveVerticalLevels") -> None:
        ...
        plotter = _plotter()
        mock_proc = _make_proc(_make_ds())
        mock_proc.get_vertical_levels.return_value = np.linspace(
            100000.0, 10000.0, N_VERT
        )
        _, coord = plotter._resolve_vertical_levels(
            mock_proc, "temperature", "height", 0
        )
...
class TestExtractCrossSectionCoords:
    ...
    def test_path_outside_domain_prints_warning(
        self: "TestExtractCrossSectionCoords", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
        ...
        ds = _make_ds()
        mock_proc = _make_proc(ds)
        mock_proc.extract_2d_coordinates_for_variable.side_effect = RuntimeError(
            "use lonCell"
        )

        path_lons = np.array([-150.0, -120.0])
        path_lats = np.array([60.0, 70.0])
    ...
    def test_path_inside_domain_no_warning(
        self: "TestExtractCrossSectionCoords", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
        ...
        path_lons = np.array([-98.0, -92.0])
        path_lats = np.array([37.0, 42.0])
        plotter._extract_cross_section_coords(
            mock_proc, "temperature", path_lons, path_lats
        )
        captured = capsys.readouterr()
...
class TestUnwrapDatasetVar:
    ...
    def test_no_vert_levels_dim_returns_none_vert_dim(
        self: "TestUnwrapDatasetVar",
    ) -> None:
        """
        ...
        ds = xr.Dataset(
            {
                "w": (
                    ...
                    np.ones((N_TIME, N_CELLS, N_VERT + 1)),
                ),
            }
        )
        mock_proc = _make_proc(ds)
...
class TestInterpolateAllLevels:
    ...
    def test_read_paths_match(self: "TestInterpolateAllLevels") -> None:
        """
        This test verifies that all three read paths in _interpolate_all_levels produce exactly the same cross-section: the reduced contiguous-range read (default), the full-grid slab read (forced by stubbing the reduced read to None), and the per-level read (forced by stubbing both reads to None). It builds a 3D variable with NaNs and asserts the three results are identical including NaN placement, locking in the equivalence of the I/O optimizations with the original per-level behaviour.
        ...
        with patch.object(plotter, "_extract_variable_cells", return_value=None):
            via_slab = plotter._interpolate_all_levels(var_da, **kwargs)

        with (
            patch.object(plotter, "_extract_variable_cells", return_value=None),
            patch.object(plotter, "_extract_variable_slab", return_value=None),
        ):
            via_per_level = plotter._interpolate_all_levels(var_da, **kwargs)

        assert np.array_equal(via_reduced, via_slab, equal_nan=True)
        assert np.array_equal(via_slab, via_per_level, equal_nan=True)

    def test_per_level_exception_prints_warning(
        self: "TestInterpolateAllLevels", capsys: pytest.CaptureFixture
    ) -> None:
        """
    ...
    def test_all_nan_result_prints_warning(
        self: "TestInterpolateAllLevels", capsys: pytest.CaptureFixture
    ) -> None:
        """
        ...
        with (
            ...
            patch.object(plotter, "_extract_variable_slab", return_value=None),
            patch.object(
                plotter, "_extract_level_data", side_effect=RuntimeError("fail")
            ),
        ):
            result = plotter._interpolate_all_levels(
                var_da,
...
class TestGenerateGreatCirclePath:
    ...
    def test_zero_distance_path_filled_with_start_point(
        self: "TestGenerateGreatCirclePath",
    ) -> None:
        """
        ...
        plotter = _plotter()
        same = (-100.0, 40.0)
        lons, lats, dists = plotter._generate_great_circle_path(
            same, same, num_points=5
        )
        assert len(lons) == 5
...
class TestInterpolateAlongPath:
    ...
    def test_xarray_grid_data_converted(self: "TestInterpolateAlongPath") -> None:
        ...
        grid_data = xr.DataArray(
            np.array([10.0, 20.0, 30.0, 40.0, 50.0]), dims=["cell"]
        )

        result = plotter._interpolate_along_path(
            np.linspace(-100.0, -90.0, 5),
            np.linspace(35.0, 45.0, 5),
            grid_data,
            np.array([-98.0, -95.0, -92.0]),
            np.array([37.0, 40.0, 43.0]),
        )
    ...
    def test_all_nan_grid_data_returns_nan_array(
        self: "TestInterpolateAlongPath",
    ) -> None:
        """
...
class TestComputeVarLevels:
    ...
    def test_temperature_branch(self: "TestComputeVarLevels") -> None:
        ...
        """
        plotter = _plotter()
        levels = plotter._compute_var_levels(
            "temperature", "temperature", 250.0, 310.0, 60.0
        )
        assert levels[0] <= 250.0
        assert levels[-1] >= 310.0
    ...
    def test_temperature_small_range_step2(self: "TestComputeVarLevels") -> None:
        ...
        plotter = _plotter()
        levels = plotter._compute_var_levels(
            "temperature", "temperature", 280.0, 295.0, 15.0
        )
    ...
    def test_pressure_positive_min_logspace(self: "TestComputeVarLevels") -> None:
        ...
        plotter = _plotter()
        levels = plotter._compute_var_levels(
            "pressure", "pressure", 100.0, 1000.0, 900.0
        )
    ...
    def test_pressure_nonpositive_min_linspace(self: "TestComputeVarLevels") -> None:
        ...
        plotter = _plotter()
        levels = plotter._compute_var_levels(
            "pressure", "pressure", -100.0, 1000.0, 1100.0
        )
    ...
    def test_wind_nonnegative_linspace(self: "TestComputeVarLevels") -> None:
        ...
        plotter = _plotter()
        levels = plotter._compute_var_levels(
            "wind_speed", "wind_speed", 0.0, 20.0, 20.0
        )
        assert len(levels) == 15
    ...
    def test_wind_symmetric_levels(self: "TestComputeVarLevels") -> None:
        ...
        """
        plotter = _plotter()
        levels = plotter._compute_var_levels("wind", "wind", -15.0, 25.0, 40.0)
    ...
    def test_u_var_name_triggers_wind_branch(self: "TestComputeVarLevels") -> None:
        ...
        """
        plotter = _plotter()
        levels = plotter._compute_var_levels("u", "u850", 0.0, 30.0, 30.0)
        assert len(levels) == 15
...
class TestGetDefaultLevels:
    ...
    def test_xarray_input(self: "TestGetDefaultLevels") -> None:
        ...
        """
        plotter = _plotter()
        da = xr.DataArray(np.linspace(250.0, 310.0, 15), dims=["level"])
    ...
    def test_list_input(self: "TestGetDefaultLevels") -> None:
        ...
        """
        plotter = _plotter()
        levels = plotter._get_default_levels(
            [[250.0, 260.0], [270.0, 280.0]], "temperature"
        )
        assert len(levels) > 0
...
class TestExtractHeightFromDataset:
    ...
    def test_var_in_dataset_extracts_values(
        self: "TestExtractHeightFromDataset",
    ) -> None:
        """
        ...
        """
        plotter = _plotter()
        height_arr = np.linspace(0.0, 14000.0, N_VERT)
        ds = xr.Dataset(
            {
                "zgrid": (
                    ["Time", "nCells", "nVertLevels"],
        ...
        result = plotter._extract_height_from_dataset(
            mock_proc, 0, vertical_coords, "zgrid"
        )
        assert result is not None
        assert len(result) == N_VERT - 1

    def test_exact_length_match_returns_directly(
        self: "TestExtractHeightFromDataset",
    ) -> None:
        """
    ...
    def test_length_mismatch_uses_interp(self: "TestExtractHeightFromDataset") -> None:
        ...
        plotter = _plotter()
        n_vert_ds = N_VERT + 3
        height_arr = np.linspace(0.0, 14000.0, n_vert_ds)

        ds = xr.Dataset(
            {
                "zgrid": (
                    ["Time", "nCells", "nVertLevelsExt"],
                    np.tile(height_arr, (N_TIME, N_CELLS, 1)),
                ),
            }
        )
    ...
    def test_outer_except_returns_none(self: "TestExtractHeightFromDataset") -> None:
        ...
        mock_proc.dataset.data_vars.__contains__ = Mock(
            side_effect=RuntimeError("boom")
        )
        result = plotter._extract_height_from_dataset(
            mock_proc, 0, np.ones(N_VERT), "zgrid"
        )
...
class TestTryExtractHeightKm:
    ...
    def test_exception_in_extraction_returns_none(
        self: "TestTryExtractHeightKm",
    ) -> None:
        """
...
class TestPressureToHeightApprox:
    ...
    def test_exception_returns_pa_over_100(self: "TestPressureToHeightApprox") -> None:
        ...
        """
        plotter = _plotter()
        vertical_coords = np.array([100000.0, 85000.0, 50000.0])
        with patch.object(
            plotter,
            "_std_atm_pressure_to_height",
            side_effect=RuntimeError("std atm failed"),
        ):
            result, coord_type = plotter._pressure_to_height_approx(vertical_coords)
        np.testing.assert_allclose(result, vertical_coords / 100.0)
...
class TestApplyStandardPressureTicks:
    ...
    def test_no_ax_returns_without_error(
        self: "TestApplyStandardPressureTicks",
    ) -> None:
        """
        ...
        plotter._apply_standard_pressure_ticks(np.array([100.0, 500.0, 1000.0]))

    def test_exception_during_locator_is_silenced(
        self: "TestApplyStandardPressureTicks",
    ) -> None:
        """
        ...
        try:
            plotter.ax.yaxis.set_major_locator = Mock(
                side_effect=RuntimeError("locator error")
            )
            plotter._apply_standard_pressure_ticks(np.array([100.0, 500.0, 1000.0]))
        finally:
...
class TestSetupPressureAxis:
    ...
    def test_empty_array_exception_returns_early(
        self: "TestSetupPressureAxis", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
    ...
    def test_nonpositive_vmin_returns_early(
        self: "TestSetupPressureAxis", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
        ...
        plotter.fig, plotter.ax = plt.subplots()
        try:
            plotter._setup_pressure_axis(np.array([-100.0, 0.0, 500.0]))
            captured = capsys.readouterr()
...
class TestApplyXAxisFormatting:
    ...
    def test_formatter_exception_silenced(self: "TestApplyXAxisFormatting") -> None:
        ...
        plotter.fig, plotter.ax = plt.subplots()
        try:
            plotter.ax.xaxis.set_major_formatter = Mock(
                side_effect=RuntimeError("fmt error")
            )
            plotter._apply_x_axis_formatting(np.array([-100.0, -90.0]))
        finally:
            plt.close(plotter.fig)
...
class TestFormatCrossSectionAxes:
    ...
    def _setup_plotter_with_ax(
        self: "TestFormatCrossSectionAxes",
    ...
    def test_pressure_pa_branch(self: "TestFormatCrossSectionAxes") -> None:
        ...
        try:
            plotter._format_cross_section_axes(
                longitudes=np.array([-100.0, -90.0]),
                vertical_coords=np.linspace(10000.0, 100000.0, N_VERT),
    ...
    def test_height_m_branch(self: "TestFormatCrossSectionAxes") -> None:
        ...
        try:
            plotter._format_cross_section_axes(
                longitudes=np.array([-100.0, -90.0]),
                vertical_coords=np.linspace(0.0, 14000.0, N_VERT),
...
class TestGetTimeString:
    ...
    def test_dataset_has_time_attribute_formats_correctly(
        self: "TestGetTimeString",
    ) -> None:
        """
    ...
    def test_no_time_attribute_returns_index_string(self: "TestGetTimeString") -> None:
        ...
        plotter = _plotter()
        ds = xr.Dataset(
            {"temperature": (["nCells", "nVertLevels"], np.ones((N_CELLS, N_VERT)))}
        )
        mock_proc = _make_proc(ds)
...
class TestResolveBatchTimeStr:
    ...
    def test_time_parse_exception_returns_fallback(
        self: "TestResolveBatchTimeStr",
    ) -> None:
        """
        ...
        with patch(
            "mpasdiag.visualization.cross_section.pd.to_datetime",
...
class TestCreateBatchCrossSectionPlots:
    ...
    def test_invalid_processor_type_raises(
        self: "TestCreateBatchCrossSectionPlots", tmp_path: "Path"
    ) -> None:
        """
