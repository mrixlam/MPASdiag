...
class TestComparisonPlot:
    ...
    def test_comparison_plot_basic(self: "TestComparisonPlot") -> None:
        ...
        fig, axes = self.plotter.create_precipitation_comparison_plot(
            self.lon, self.lat, self.precip1, self.precip2, -180, 180, -90, 90
        )
...
class TestSavePlot:
    ...
    def test_save_plot_creates_directory(
        self: "TestSavePlot",
        mpas_coordinates: tuple[np.ndarray, np.ndarray],
        mpas_precip_data: np.ndarray,
    ) -> None:
...
class TestEdgeCases:
    ...
    @pytest.fixture(autouse=True)
    def setup_method(
        self: "TestEdgeCases", mpas_coordinates: tuple[np.ndarray, np.ndarray]
    ) -> None:
        """
...
class TestPrepareOverlayData:
    ...
    def test_overlay_data_negative_clipping(self: "TestPrepareOverlayData") -> None:
        ...
        with patch("sys.stdout", captured):
            result = self.plotter._prepare_overlay_data(
                lon, lat, data, "test_var", None, "scatter"
            )
    ...
    def test_overlay_data_unit_conversion(self: "TestPrepareOverlayData") -> None:
        ...
        with patch("sys.stdout", captured):
            result = self.plotter._prepare_overlay_data(
                lon, lat, data, "precipitation", "mm", "scatter"
            )
    ...
    def test_overlay_data_high_mean_warning(self: "TestPrepareOverlayData") -> None:
        ...
        with patch("sys.stdout", captured):
            result = self.plotter._prepare_overlay_data(
                lon, lat, data, "precip_var", None, "scatter"
            )

        assert "may not be in mm" in captured.getvalue().lower() or len(result[0]) > 0

    def test_overlay_data_contour_mask_differs_from_scatter(
        ...
        result_scatter = self.plotter._prepare_overlay_data(
            lon, lat, data, "test", None, "scatter"
        )
        result_contour = self.plotter._prepare_overlay_data(
            lon, lat, data, "test", None, "contourf"
        )
...
class TestContourPlotCreation:
    ...
    def setup(self: "TestContourPlotCreation") -> None:
        ...
        self.plotter.ax = self.plotter.fig.add_subplot(
...
class TestAddPrecipitationOverlay:
    ...
    def test_overlay_uses_data_bounds_when_none_provided(
        self: "TestAddPrecipitationOverlay",
    ) -> None:
        """
...
class TestPrecipitationOverlayEdgeCases:
    ...
    def test_prepare_overlay_data_negative_clipping(
        self: "TestPrecipitationOverlayEdgeCases", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
    ...
    def test_prepare_overlay_data_with_unit_conversion(
        self: "TestPrecipitationOverlayEdgeCases", capsys: "pytest.CaptureFixture"
    ) -> None:
        """
    ...
    def test_prepare_overlay_data_xarray_inputs(
        self: "TestPrecipitationOverlayEdgeCases",
    ) -> None:
        """
...
class TestPrecipitationPlotterHelpers:
    ...
    @pytest.fixture(autouse=True)
    def setup_plotter(
        self: "TestPrecipitationPlotterHelpers",
    ...
    def test_create_precip_colormap_all_periods(
        self: "TestPrecipitationPlotterHelpers", accum: str
    ) -> None:
        """
    ...
    def test_convert_precip_units_xarray_input(
        self: "TestPrecipitationPlotterHelpers",
    ) -> None:
        """
    ...
    def test_convert_precip_units_negative_clipped(
        self: "TestPrecipitationPlotterHelpers",
    ) -> None:
        """
...
class TestTimeAnnotationSource:
    ...
    def test_metadata_drives_annotation_over_config(
        ...
        assert (
            annotation
            == "Accumulation: 2024-09-16 23:00 UTC to 2024-09-17 00:00 UTC (1 h)"
        )
        plt.close(fig)

    def test_explicit_time_end_takes_precedence_over_metadata(
        self: "TestTimeAnnotationSource",
    ) -> None:
        """
        ...
        assert (
            annotation
            == "Accumulation: 2024-09-18 05:00 UTC to 2024-09-18 06:00 UTC (1 h)"
        )
        plt.close(fig)
...
class TestResolveAccumulationTimes:
    ...
    @staticmethod
    def _data_array_with_meta(valid_time: str, accum_start: str) -> xr.DataArray:
        """
    ...
    def test_explicit_end_matching_metadata_uses_exact_start(
        self: "TestResolveAccumulationTimes",
    ) -> None:
        """
        ...
        da = self._data_array_with_meta("2024-09-17T00:00:00", "2024-09-16T23:30:00")
            ...
            time_end=datetime(2024, 9, 17, 0, 0),
            time_start=None,
            data_array=da,
            dataset=None,
            config=None,
    ...
    def test_explicit_end_mismatching_metadata_drops_start(
        self: "TestResolveAccumulationTimes",
    ) -> None:
        """
    ...
    def test_metadata_used_when_no_explicit_end(
        self: "TestResolveAccumulationTimes",
    ) -> None:
        """
