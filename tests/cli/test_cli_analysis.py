...
class TestAnalysisExecution:
    ...
    def test_run_analysis_unknown_analysis_type(self: "TestAnalysisExecution") -> None:
        ...
        config = MPASConfig(
            grid_file="test.nc", data_dir="data/", analysis_type="invalid_type"
        )
    ...
    def test_run_analysis_recognizes_precipitation_alias(
        self: "TestAnalysisExecution",
    ) -> None:
        """
        ...
        for alias in ["precipitation", "precip", "rain"]:
            config = MPASConfig(
                grid_file="test.nc", data_dir="data/", analysis_type=alias
            )

            with patch.object(
                cli, "_run_precipitation_analysis", return_value=True
            ) as mock_method:
                result = cli.run_analysis(config)
                mock_method.assert_called_once_with(config)
                assert result is True

    def test_run_analysis_recognizes_surface_alias(
        self: "TestAnalysisExecution",
    ) -> None:
        """
        ...
        for alias in ["surface", "surf", "2d"]:
            ...
            with patch.object(
                cli, "_run_surface_analysis", return_value=True
            ) as mock_method:
                result = cli.run_analysis(config)
                mock_method.assert_called_once_with(config)
                assert result is True
    ...
    def test_run_analysis_recognizes_wind_alias(self: "TestAnalysisExecution") -> None:
        ...
        for alias in ["wind", "vector", "winds"]:
            ...
            with patch.object(
                cli, "_run_wind_analysis", return_value=True
            ) as mock_method:
                result = cli.run_analysis(config)
                mock_method.assert_called_once_with(config)
                assert result is True
    ...
    def test_run_analysis_recognizes_cross_alias(self: "TestAnalysisExecution") -> None:
        ...
        for alias in ["cross", "xsec", "3d", "vertical"]:
            ...
            with patch.object(
                cli, "_run_cross_analysis", return_value=True
            ) as mock_method:
                result = cli.run_analysis(config)
                mock_method.assert_called_once_with(config)
                assert result is True

    def test_run_analysis_recognizes_overlay_alias(
        self: "TestAnalysisExecution",
    ) -> None:
        """
        ...
        for alias in ["overlay", "complex", "multi", "composite"]:
            ...
            with patch.object(
                cli, "_run_overlay_analysis", return_value=True
            ) as mock_method:
                result = cli.run_analysis(config)
                mock_method.assert_called_once_with(config)
                assert result is True
...
class TestCrossSectionAnalysis:
    ...
    def test_parse_args_cross_with_vertical_coord(
        self: "TestCrossSectionAnalysis",
    ) -> None:
        """
...
class TestCrossSectionWorkflows:
    ...
    def test_cross_section_single_time_with_logger(
        self: "TestCrossSectionWorkflows",
    ) -> None:
        """
