...
class TestErrorHandlingAndExceptions:
    ...
    def test_run_analysis_keyboard_interrupt(
        self: "TestErrorHandlingAndExceptions",
    ) -> None:
        """
    ...
    def test_run_analysis_generic_exception_no_verbose(
        self: "TestErrorHandlingAndExceptions",
    ) -> None:
        """
    ...
    def test_run_analysis_generic_exception_with_verbose(
        self: "TestErrorHandlingAndExceptions",
    ) -> None:
        """
    ...
    def test_validate_config_data_dir_not_directory(
        self: "TestErrorHandlingAndExceptions", grid_file: str
    ) -> None:
        """
        ...
        config = MPASConfig(
            grid_file=grid_file, data_dir=grid_file, analysis_type="surface"
        )

        result = cli.validate_config(config)
        assert result is False

    def test_validate_config_cross_section_missing_coords(
        self: "TestErrorHandlingAndExceptions",
    ) -> None:
        """
    ...
    def test_validate_config_without_logger(
        self: "TestErrorHandlingAndExceptions",
    ) -> None:
        """
    ...
    def test_run_analysis_exception_handling_with_verbose(
        self: "TestErrorHandlingAndExceptions",
    ) -> None:
        """
        ...
        with patch.object(
            cli, "_run_precipitation_analysis", side_effect=Exception("Test error")
        ):
            result = cli.run_analysis(config)
            assert result is False
