...
class TestGetVerticalLevels:
    ...
    def _create_mock_dataset_with_pressure_components(
        self: "TestGetVerticalLevels",
    ) -> Any:
        """
        ...
        mock_ds.__getitem__.side_effect = make_getitem(
            {"pressure_base": mock_pressure_base, "pressure_p": mock_pressure_p},
            default=mock_var,
        )
        mock_ds.__contains__.side_effect = make_contains(
            ["theta", "pressure_p", "pressure_base"]
        )
...
class TestAddSpatialCoordinates:
    ...
    def test_add_coordinates_to_real_dataset(self: "TestAddSpatialCoordinates") -> None:
        ...
        has_coords = any(
            c in processor.dataset.coords or c in processor.dataset.data_vars
