...
class TestPrecipitationWorker:
    ...
    def test_precipitation_worker_with_weights_dir(
        self: "TestPrecipitationWorker",
    ) -> None:
        """
...
class TestWindWorker:
    ...
    def setup_method(self: "TestWindWorker") -> None:
        ...
        self.mock_processor.dataset = self.mock_dataset
        self.mock_processor.get_2d_variable_data.side_effect = (
            lambda var, idx: self.mock_dataset[var].isel(Time=idx)
        )
...
class TestCrossSectionWorker:
    ...
    def test_cross_section_worker_uppercase_format(
        self: "TestCrossSectionWorker",
    ) -> None:
        """
...
class TestWorkerCacheKeyErrorFallback:
    ...
    def setup_method(self: "TestWorkerCacheKeyErrorFallback") -> None:
        ...
        self.mock_processor_precip.extract_2d_coordinates_for_variable.return_value = (
            lon,
            lat,
        )
        ...
        self.mock_processor_surface.extract_spatial_coordinates.return_value = (
            lon,
            lat,
        )
        ...
        self.mock_processor_wind.extract_2d_coordinates_for_variable.return_value = (
            lon,
            lat,
        )
    ...
    def test_precipitation_worker_no_time_dim_fallback(
        ...
        proc.dataset = xr.Dataset(
            {
                "rainnc": xr.DataArray(np.zeros((3, 100)), dims=["step", "nCells"]),
            }
        )
