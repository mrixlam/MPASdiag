...
def test_error_handling() -> None:
    ...
    assert len(successes) == pytest.approx(
        9
    ), f"Expected 9 successes, got {len(successes)}"
...
def test_n_workers_argument() -> None:
    ...
    print(
        f"✓ n_workers=1: Wall time = {time_1_worker:.2f}s, Pool size = {manager_1.size}"
    )
    ...
    print(
        f"✓ n_workers=4: Wall time = {time_4_workers:.2f}s, Pool size = {manager_4.size}"
    )
    ...
    print(
        f"✓ n_workers=None: Wall time = {time_auto:.2f}s, Pool size = {manager_auto.size}"
    )
...
def test_n_workers_with_cli_integration() -> None:
    ...
    for n_workers_arg, description in test_cases:
        ...
        manager = MPASParallelManager(
            n_workers=n_workers_arg, backend="multiprocessing", verbose=False
        )
