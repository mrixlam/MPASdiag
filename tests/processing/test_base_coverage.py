...
def _minimal_ds(n_time: int = 2, n_cells: int = 10) -> xr.Dataset:
    """
...
class TestInitGridFileMissing:
    ...
    def test_raises_file_not_found_for_nonexistent_grid(
        self: "TestInitGridFileMissing", tmp_path: "Path"
    ) -> None:
        """
...
class TestFindFilesByPatternErrors:
    ...
    def test_raises_when_no_files_match_pattern(
        self: "TestFindFilesByPatternErrors", tmp_path: "Path"
    ) -> None:
        """
    ...
    def test_raises_when_only_one_file_found(
        self: "TestFindFilesByPatternErrors", tmp_path: "Path"
    ) -> None:
        """
...
class TestValidateFiles:
    ...
    def test_raises_for_nonexistent_file(
        self: "TestValidateFiles", tmp_path: "Path"
    ) -> None:
        """
    ...
    def test_raises_for_unreadable_file(
        self: "TestValidateFiles", tmp_path: "Path"
    ) -> None:
        """
    ...
    def test_returns_list_of_valid_files(
        self: "TestValidateFiles", tmp_path: "Path"
    ) -> None:
        """
...
class TestPrepareChunkingConfig:
    ...
    def test_custom_chunks_returns_time_only_open_chunks(
        self: "TestPrepareChunkingConfig",
    ) -> None:
        """
    ...
    def test_none_chunks_returns_default_config(
        self: "TestPrepareChunkingConfig",
    ) -> None:
        """
        This test verifies that _prepare_chunking_config with None returns a default open_chunks dict with 'Time' set to 1, and a full_chunks dict that chunks along Time only. The test confirms that when no custom chunking configuration is provided, the method returns chunking that follows the one-file-per-timestep MPAS layout without rechunking spatial dimensions: splitting nCells below the on-disk HDF5 chunk size would force every read to re-decompress the same stored chunks (read amplification), which multiplies across parallel ranks. This ensures that the method correctly provides sensible defaults for chunking when no custom configuration is specified.
        ...
        assert full_c == {"Time": 1}
        assert "nCells" not in full_c
...
class TestLoadMultifileDatasetDropVars:
    ...
    def test_drop_variables_passed_to_open_mfdataset(
        self: "TestLoadMultifileDatasetDropVars", tmp_path: "Path"
    ) -> None:
        """
...
class TestApplyChunking:
    ...
    def test_returns_dataset_unchanged_when_chunks_is_none(
        self: "TestApplyChunking",
    ) -> None:
        """
...
class TestCreateUxarrayDataset:
    ...
    def test_raises_when_uxgrid_is_none(
        self: "TestCreateUxarrayDataset", tmp_path: "Path"
    ) -> None:
        """
        ...
        with patch(
            "mpasdiag.processing.base.ux.open_dataset", return_value=mock_grid_ds
        ):
...
class TestAttemptFallbackLoad:
    ...
    def test_returns_xarray_type_and_sets_instance_attrs(
        self: "TestAttemptFallbackLoad", tmp_path: "Path"
    ) -> None:
        """
    ...
    def test_verbose_calls_print_loading_success(
        self: "TestAttemptFallbackLoad", tmp_path: "Path"
    ) -> None:
        """
...
class TestDiscoverDataFilesDefaultFallback:
    ...
    def test_calls_find_files_by_pattern_when_no_custom_finders(
        self: "TestDiscoverDataFilesDefaultFallback", tmp_path: "Path"
    ) -> None:
        """
        ...
        with patch.object(
            proc, "_find_files_by_pattern", return_value=fake_files
        ) as mock_find:
...
class TestLoadDataSelectiveVariables:
    ...
    def _setup_load_data_mocks(
        self: "TestLoadDataSelectiveVariables",
        ...
        self._mocks = {
            "discover": patch.object(
                proc, "_discover_data_files", return_value=fake_files
            ),
            "parse": patch(
        ...
        return patch.object(
            proc, "_attempt_primary_load", return_value=(result_ds, "xarray")
        )

    def test_computes_drop_variables_from_probe(
        self: "TestLoadDataSelectiveVariables", tmp_path: "Path"
    ) -> None:
        """
        ...
        with (
            self._mocks["discover"],
            self._mocks["parse"],
            self._mocks["chunks"],
            self._mocks["open_ds"],
            primary_mock as mock_primary,
        ):
    ...
    def test_verbose_prints_selective_loading_message(
        self: "TestLoadDataSelectiveVariables", tmp_path: "Path"
    ) -> None:
        """
    ...
    def test_probe_exception_sets_drop_variables_to_none(
        self: "TestLoadDataSelectiveVariables", tmp_path: "Path"
    ) -> None:
        """
        ...
        with (
            patch.object(proc, "_discover_data_files", return_value=fake_files),
            patch(
                "mpasdiag.processing.base.MPASDateTimeUtils.parse_file_datetimes",
                return_value=times,
            ),
            patch.object(
                proc, "_prepare_chunking_config", return_value=({"Time": 1}, None)
            ),
...
class TestLoadDataAllStrategiesFail:
    ...
    def test_sys_exit_when_every_loading_strategy_raises(
        self: "TestLoadDataAllStrategiesFail", tmp_path: "Path"
    ) -> None:
        """
        ...
        with (
            ...
            patch.object(
                proc, "_attempt_primary_load", side_effect=Exception("primary fail")
            ),
            patch.object(
                proc, "_attempt_fallback_load", side_effect=Exception("fallback fail")
            ),
            patch.object(
                proc, "_load_single_file_fallback", side_effect=Exception("single fail")
            ),
        ):
...
class TestSelectFallbackFile:
    ...
    def test_returns_reference_file_when_it_exists(
        self: "TestSelectFallbackFile", tmp_path: "Path"
    ) -> None:
        """
    ...
    def test_returns_first_data_file_when_reference_is_empty(
        self: "TestSelectFallbackFile", tmp_path: "Path"
    ) -> None:
        """
...
class TestAttemptSingleFileLoad:
    ...
    def test_uxarray_path_sets_dataset_and_data_type(
        self: "TestAttemptSingleFileLoad", tmp_path: "Path"
    ) -> None:
        """
    ...
    def test_xarray_fallback_when_uxarray_raises(
        self: "TestAttemptSingleFileLoad", tmp_path: "Path"
    ) -> None:
        """
        ...
        with patch.object(
            proc, "_load_single_file_uxarray", side_effect=Exception("ux fail")
        ):
            with patch.object(
                proc, "_load_single_file_xarray", return_value=mock_xr_ds
            ):
                _, dtype = proc._attempt_single_file_load(str(tmp_path / "f.nc"))
...
class TestGetAvailableVariables:
    ...
    def test_returns_list_of_data_variable_names(
        self: "TestGetAvailableVariables",
    ) -> None:
        """
...
class TestValidateGeographicExtent:
    ...
    def test_invalid_lon_min_returns_false(
        self: "TestValidateGeographicExtent",
    ) -> None:
        """
    ...
    def test_invalid_lon_max_returns_false(
        self: "TestValidateGeographicExtent",
    ) -> None:
        """
    ...
    def test_invalid_lat_min_returns_false(
        self: "TestValidateGeographicExtent",
    ) -> None:
        """
        ...
        """
        assert (
            self.proc.validate_geographic_extent((-10.0, 10.0, -100.0, 10.0)) is False
        )

    def test_invalid_lat_max_returns_false(
        self: "TestValidateGeographicExtent",
    ) -> None:
        """
    ...
    def test_lon_min_ge_lon_max_returns_false(
        self: "TestValidateGeographicExtent",
    ) -> None:
        """
    ...
    def test_lat_min_ge_lat_max_returns_false(
        self: "TestValidateGeographicExtent",
    ) -> None:
        """
...
class TestExtractSpatialCoordinates:
    ...
    def test_raises_when_no_lon_lat_vars_present(
        self: "TestExtractSpatialCoordinates",
    ) -> None:
        """
    ...
    def test_radian_coordinates_are_converted_to_degrees(
        self: "TestExtractSpatialCoordinates",
    ) -> None:
        """
...
class TestLoadGridFileProbeException:
    ...
    def test_probe_exception_falls_through_to_actual_load(
        self: "TestLoadGridFileProbeException",
    ) -> None:
        """
...
class TestApplyCoordinateUpdates:
    ...
    def test_verbose_prints_no_additional_coords_message(
        self: "TestApplyCoordinateUpdates",
    ) -> None:
        """
    ...
    def test_coords_added_when_coords_to_add_is_provided(
        self: "TestApplyCoordinateUpdates",
    ) -> None:
        """
...
class TestAddSpatialCoordsHelperException:
    ...
    def test_warning_printed_and_original_ds_returned_on_exception(
        self: "TestAddSpatialCoordsHelperException",
    ) -> None:
        """
        ...
        ds = _minimal_ds()
        with patch.object(
            proc, "_load_grid_file", side_effect=Exception("grid load failed")
        ):
            out = StringIO()
            with redirect_stdout(out):
...
class TestParseFileDatetimes:
    ...
    def test_delegates_to_datetime_utils_and_returns_list(
        self: "TestParseFileDatetimes", tmp_path: "Path"
    ) -> None:
        """
        ...
        with patch(
            ...
            result = proc.parse_file_datetimes(
                [
                    str(tmp_path / "diag_2024010100.nc"),
                    str(tmp_path / "diag_2024010200.nc"),
                ]
            )
...
class TestValidateTimeParameters:
    ...
    def test_delegates_to_datetime_utils_with_loaded_dataset(
        self: "TestValidateTimeParameters",
    ) -> None:
        """
