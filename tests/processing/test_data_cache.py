...
from mpasdiag.processing.data_cache import (
    MPASDataCache,
    CachedVariable,
    get_global_cache,
    clear_global_cache,
...
class TestLoadVariableData:
    ...
    def setup_method(self: "TestLoadVariableData", mpas_3d_processor: Any) -> Any:
        """
    ...
    def test_load_variable_with_time_indexing(self: "TestLoadVariableData") -> None:
        ...
        if var_name:
            ...
            assert cached.time_index == pytest.approx(0, abs=1e-3)
            assert "Time" not in (
                cached.data.shape if hasattr(cached.data, "shape") else []
            )

    def test_load_variable_with_nvertlevels_indexing(
        self: "TestLoadVariableData",
    ) -> None:
        """
        ...
        if var_name:
            cache.load_variable_data(
                processor.dataset, var_name, time_index=0, level_index=0
            )
            cached = cache.get_variable_data(var_name, time_index=0, level_index=0)
            assert cached.level_index == pytest.approx(0, abs=1e-3)

    def test_load_variable_with_nvertlevelsp1_indexing(
        self: "TestLoadVariableData",
    ) -> None:
        """
    ...
    def test_load_variable_cache_size_limit_and_eviction(
        self: "TestLoadVariableData",
    ) -> None:
        """
...
class TestRealDataIntegration:
    ...
    @pytest.fixture(autouse=True)
    def setup_method(self: "TestRealDataIntegration", mpas_3d_processor: Any) -> Any:
        """
