...
class TestPrecipitationWorkerCacheException:
    ...
    def test_precipitation_worker_cache_load_exception(
        self: "TestPrecipitationWorkerCacheException", temp_output_dir: Path
    ) -> None:
        """
        ...
        mock_dataset = xr.Dataset(
            {
                ...
                "lonCell": xr.DataArray(
                    np.linspace(-120, -80, n_cells), dims=["nCells"]
                ),
        ...
        mock_cache.load_coordinates_from_dataset.side_effect = RuntimeError(
            "Cache load failed"
        )
        ...
        orig_plotter, orig_diag = (
            _pw.MPASPrecipitationPlotter,
            _pw.PrecipitationDiagnostics,
        )
        ...
        try:
            ...
            _pw.MPASPrecipitationPlotter, _pw.PrecipitationDiagnostics = (
                orig_plotter,
                orig_diag,
            )
...
class TestPrecipitationWorkerTimeString:
    ...
    def test_precipitation_worker_no_time_dimension(
        self: "TestPrecipitationWorkerTimeString", temp_output_dir: Path
    ) -> None:
        """
        ...
        mock_dataset = xr.Dataset(
            {
                "rainnc": xr.DataArray(_RNG.uniform(0, 50, n_cells), dims=["nCells"]),
                "lonCell": xr.DataArray(
                    np.linspace(-120, -80, n_cells), dims=["nCells"]
                ),
                "latCell": xr.DataArray(np.linspace(30, 50, n_cells), dims=["nCells"]),
...
class TestPrecipitationWorkerCustomTitle:
    ...
    def test_precipitation_worker_custom_title_template(
        self: "TestPrecipitationWorkerCustomTitle", temp_output_dir: Path
    ) -> None:
        """
        ...
        custom_template = (
            "Custom Precip: {var_name} | Time: {time_str} | Period: {accum_period}"
        )
...
class TestSurfaceWorkerCacheException:
    ...
    def test_surface_worker_cache_load_exception(
        self: "TestSurfaceWorkerCacheException", temp_output_dir: Path
    ) -> None:
        """
...
class TestWindWorkerTimeString:
    ...
    def test_wind_worker_no_time_dimension(
        self: "TestWindWorkerTimeString", temp_output_dir: Path
    ) -> None:
        """
        ...
        mock_processor.dataset = mock_dataset
        mock_processor.get_2d_variable_data.side_effect = lambda var, idx: mock_dataset[
            var
        ]
...
class TestWindWorkerCacheException:
    ...
    def test_wind_worker_cache_load_exception(
        self: "TestWindWorkerCacheException", temp_output_dir: Path
    ) -> None:
        """
...
class TestCrossSectionWorkerMultipleFormats:
    ...
    def test_cross_section_worker_multiple_formats(
        self: "TestCrossSectionWorkerMultipleFormats", temp_output_dir: Path
    ) -> None:
        """
...
class TestProcessParallelResultsFailures:
    ...
    def test_process_parallel_results_with_failures(
        self: "TestProcessParallelResultsFailures", temp_output_dir: Path
    ) -> None:
        """
        ...
        results = [
            ...
            TaskResult(task_id=1, success=False, error="Processing failed"),
            TaskResult(
                task_id=2,
                success=True,
                result={
