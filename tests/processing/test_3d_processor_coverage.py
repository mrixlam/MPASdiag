...
def _synthetic_mpas_data(
    n_cells: int = 5,
    n_vert: int = 4,
    n_time: int = 1,
    with_pressure_components: bool = True,
    with_direct_pressure: bool = False,
) -> xr.Dataset:
    """
...
def _make_getitem(mapping: dict, default: Any = None) -> Any:
    """
...
class TestInterpolateAtPressure:
    ...
    def _setup(self: "TestInterpolateAtPressure", verbose: bool = False) -> tuple:
        """
    ...
    def test_verbose_prints_interpolation_info(
        self: "TestInterpolateAtPressure", capsys: pytest.CaptureFixture
    ) -> None:
        """
        ...
        assert (
            "pressure" in captured.out.lower()
            or "interpolating" in captured.out.lower()
        )
...
class TestResolveLevelIndex:
    ...
    def test_float_level_dispatches_to_float_resolution(
...
class TestGetVariableDataEarlyReturn:
    ...
    def test_interpolated_data_returned_directly(
        self: "TestGetVariableDataEarlyReturn",
    ) -> None:
        """
...
class TestGetVariableDataNdimCheck:
    ...
    def test_multidim_extraction_raises_value_error(
        self: "TestGetVariableDataNdimCheck",
    ) -> None:
        """
