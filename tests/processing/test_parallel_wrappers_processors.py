...
class TestProcessParallelResults:
    ...
    def test_process_results_mixed(self: "TestProcessParallelResults") -> None:
        ...
        results = [
            ...
            TaskResult(task_id=1, success=False, error="Test error"),
        ]
    ...
    def test_process_results_success_with_error_key(
        self: "TestProcessParallelResults",
    ) -> None:
        """
        ...
        results = [
            ...
            TaskResult(
                ...
                success=True,
                result={
                    "error": "worker encountered a problem",
                    "files": [],
                    "timings": {},
                },
            ),
...
class TestParallelPrecipitationProcessor:
    ...
    def test_create_batch_precipitation_maps_parallel(
        self: "TestParallelPrecipitationProcessor",
    ) -> None:
        """
...
class TestParallelSurfaceProcessor:
    ...
    def setup_method(self: "TestParallelSurfaceProcessor") -> None:
        ...
        self.mock_dataset = xr.Dataset(
            {
                "t2m": xr.DataArray(
                    _RNG.uniform(250, 310, (n_time, n_cells)), dims=["Time", "nCells"]
                )
            }
        )
    ...
    def test_create_batch_surface_maps_parallel(
        self: "TestParallelSurfaceProcessor",
    ) -> None:
        """
...
class TestParallelWindProcessor:
    ...
    def setup_method(self: "TestParallelWindProcessor") -> None:
        ...
        self.mock_dataset = xr.Dataset(
            {
                "u10": xr.DataArray(
                    _RNG.uniform(-10, 10, (n_time, n_cells)), dims=["Time", "nCells"]
                ),
                "v10": xr.DataArray(
                    _RNG.uniform(-10, 10, (n_time, n_cells)), dims=["Time", "nCells"]
                ),
    ...
    def test_create_batch_wind_plots_parallel(
        self: "TestParallelWindProcessor",
    ) -> None:
        """
...
class TestParallelCrossSectionProcessor:
    ...
    def test_create_batch_cross_section_plots_parallel(
        self: "TestParallelCrossSectionProcessor",
    ) -> None:
        """
...
class TestEdgeCases:
    ...
    def test_cache_load_failure(self: "TestEdgeCases") -> None:
        ...
        mock_dataset = xr.Dataset(
            {
                "rainnc": xr.DataArray(
                    _RNG.uniform(0, 50, (5, 100)), dims=["Time", "nCells"]
                )
            }
        )
...
class TestParallelPrecipitationProcessorEdgeCases:
    ...
    def test_weights_dir_and_resolution_triggers_prebuild_remapper(
        ...
        try:
            with patch(
                "mpasdiag.processing.parallel_wrappers._prebuild_remapper_mpi"
            ) as mock_pre:
                ParallelPrecipitationProcessor.create_batch_precipitation_maps_parallel(
                    processor=self.mock_processor,
                    output_dir=self.temp_dir,
...
class TestParallelSurfaceProcessorEdgeCases:
    ...
    def test_cache_preload_exception_shows_warning(
        ...
        mock_cache = Mock()
        mock_cache.load_coordinates_from_dataset.side_effect = Exception(
            "coord load failed"
        )
