...
def assert_expected_public_methods(instance: object, class_key: str) -> None:
    """
    ...
    assert not missing, f"{class_key} instance is missing public methods: {missing}"
...
def load_mpas_coords_from_processor(
    n: int = 100,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
...
def load_precip_from_diag(
    n: int = 100, var_candidates: Tuple[str, ...] = ("rainc", "rain_nc", "rain")
) -> np.ndarray:
    """
    ...
    available = (
        set(proc.dataset.data_vars.keys()) if proc.dataset is not None else set()
    )
...
def load_surface_t2m_from_diag(
    n: int = 100, var_candidates: Tuple[str, ...] = ("t2m", "air_temperature", "t_surf")
) -> np.ndarray:
    """
...
def load_wind_uv_from_diag(
    ...
    v_candidates: Tuple[str, ...] = ("v", "v10", "y_wind"),
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def load_qv_3d_from_mpasout(
    ...
    available = (
        set(proc3.dataset.data_vars.keys()) if proc3.dataset is not None else set()
    )
...
def get_mpas_data_paths() -> dict:
    ...
    paths = {
        ...
        "grid_file": (
            data_root / "grids" / "x1.10242.static.nc"
            if (data_root / "grids" / "x1.10242.static.nc").exists()
            else None
        ),
        "u240k_dir": data_root / "u240k" if (data_root / "u240k").exists() else None,
        "diag_dir": (
            data_root / "u240k" / "diag"
            if (data_root / "u240k" / "diag").exists()
            else None
        ),
        "mpasout_dir": (
            data_root / "u240k" / "mpasout"
            if (data_root / "u240k" / "mpasout").exists()
            else None
        ),
    }
...
def check_mpas_data_available() -> bool:
    ...
    return paths["grid_file"] is not None and (
        paths["diag_dir"] is not None or paths["mpasout_dir"] is not None
    )


def load_mpas_2d_processor(
    data_subdir: str = "u240k/diag", verbose: bool = False
) -> MPAS2DProcessor:
    """
...
def load_mpas_3d_processor(
    data_subdir: str = "u240k/mpasout", verbose: bool = False
) -> MPAS3DProcessor:
    """
...
def get_real_mpas_variable(
    processor: Union[MPAS2DProcessor, MPAS3DProcessor],
    variable_name: str,
    time_index: int = 0,
    ...
    if hasattr(processor, "get_3d_variable_data") and level is not None:
        da = processor.get_3d_variable_data(
            variable_name, level=level, time_index=time_index
        )
    else:
